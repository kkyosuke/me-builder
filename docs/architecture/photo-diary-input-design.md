...
写真日記の公開では、文章を書きづらい日も、本人が残したい出来事を写真で記録できることを目指します。たとえば、出かけた先の風景や食事をLINEへ送り、後から本人が見返す使い方です。まず本節の保存段階を公開し、送信した写真を保存・閲覧できる価値を検証します。

写真の前後に短いテキスト日記を送る使い方もできますが、写真と文章の受付単位は[§3.1](#31-受付条件)に従います。写真と文章を自動で1件にまとめる機能や、写真の内容を理解したAIの返答は、保存段階の公開価値として案内しません。

検証では、写真を送った後に保存結果を確認できた割合、保存した写真を本人が見返した割合、文章を書きづらい日にも記録を残せたという評価を確認します。日記本文や画像の内容を計測ログへ収集しません。公開前の残作業と完了条件は[日記入力残タスク](../development/diary-remaining-tasks.md#21-日記の写真添付を公開する)から参照します。
