...
Identity PlatformのWeb API keyは`GOOGLE_IDENTITY_PLATFORM_API_KEY`、Tenant IDは`GOOGLE_IDENTITY_PLATFORM_TENANT_ID`、OAuth Client IDは`GOOGLE_OAUTH_CLIENT_ID`、OAuth Client Secretは`GOOGLE_OAUTH_CLIENT_SECRET`へ設定します。Localはgit管理外の`.env`を使います。PreviewとProductionでは、4値を環境別GCP Secret ManagerからDirect WIFでCDへ読み込み、同じCloudflare API Worker versionへまとめてデプロイします。API keyはproject識別子であり単独では認可情報になりませんが、この構成ではserver-side専用値としてsecret配布し、Identity Toolkit APIだけへAPI制限を付けます。OAuth Client Secret、authorization code、token、subjectはworkflowの引数、ログ、artifactへ出しません。Tenant IDとOAuth Client IDもGitHub Environmentへ複製せず、環境をまたぐ設定の取り違えを防ぎます。GCS stateをruntime設定の配布元にしないIAM境界は[インフラ・システム構成](./infrastructure-architecture.md#62-gcp共通リソースの宣言境界)を正とします。

OAuth Client IDは秘密値ではありませんが、環境を誤接続しないよう他のSSO実行値と同じSecret Managerから配布します。起動時にcallbackのoriginが`BASE_URL`と一致し、ProductionではHTTPS、Localではloopback HTTPだけを許可します。
