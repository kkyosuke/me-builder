...
Identity PlatformのWeb API keyは`GOOGLE_IDENTITY_PLATFORM_API_KEY`、Tenant IDは`GOOGLE_IDENTITY_PLATFORM_TENANT_ID`、OAuth Client IDは`GOOGLE_OAUTH_CLIENT_ID`、OAuth Client Secretは`GOOGLE_OAUTH_CLIENT_SECRET`へ設定します。Localはgit管理外の`.env`を使います。PreviewとProductionでは、API keyを環境別GCP Secret ManagerからDirect WIFでCDへ読み込み、Tenant IDとOAuth Client IDはGitHub Environment `dev`または`prd`のvariable、OAuth Client Secretは同EnvironmentのsecretとしてCloudflare API Workerへデプロイします。API keyはproject識別子であり単独では認可情報になりませんが、この構成ではserver-side専用値としてsecret配布し、Identity Toolkit APIだけへAPI制限を付けます。Secret値、authorization code、token、subjectはworkflowの引数、ログ、artifactへ出しません。GCS stateをruntime secretの配布元にしないIAM境界は[インフラ・システム構成](./infrastructure-architecture.md#62-gcp共通リソースの宣言境界)を正とします。
