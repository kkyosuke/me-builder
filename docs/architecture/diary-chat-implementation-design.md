...
`dev` / `development` / `local` / `preview`では、モデルが回答へ実際に反映したBrain Itemがある場合だけ、通常返信の末尾へ`[dev] 使用したBrain Item`として分類とstatementを追記します。検索候補だっただけのItem、Brain Item ID、Source Record ID、Evidence本文は表示しません。使用0件では追加表示せず、`test`と`production`では常に表示しません。追記後の本文はLINE text messageの上限である5,000文字以内に収め、同じ本文をassistant messageへ保存して再配送時の内容を固定します。
