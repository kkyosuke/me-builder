...
sequenceDiagram
    ...
    alt 停止済み・期限切れ・会話中・本人発言あり・翌日休止・自動休止
        ...
        W->>L: 配送行の文面versionをretry key付きPush
        W->>AD: deliveredを記録
    end
```
...
`safety_route`と安全応答の最終結果をTurnへ保存し、相談本文を複製せずに安全経路の利用状況を確認できるようにします。安全経路だけの特別な保持期間は設けず、対応する会話と同じライフサイクルに従います。運用ログにはroute、結果、固定の失敗理由だけを出し、相談本文、生成本文、Account IDを含めません。
...
終端化の原因を運用者が解消した場合は、開発環境限定の`GET /api/dev/brain-vector-sync-jobs/failed`で管理者本人の終端jobを一覧し、`POST /api/dev/brain-vector-sync-jobs/:jobId/reset`または`POST /api/dev/brain-vector-sync-jobs/reset-failed`でjob ID指定またはAccount内一括のresetを行います。APIはAccount IDを入力として受け取らず、セッションから管理者本人のAccountを解決して、AccountDataの`brain.listFailedVectorSyncJobs`、`brain.resetFailedVectorSyncJob`、`brain.resetAllFailedVectorSyncJobs`を呼びます。一括操作は`dry-run`で古い順に最大25件の候補数を返し、`execute`と明示確認を受けた場合だけ同じ上限でresetします。resetの実行は認証から10分以内に限り、resetしたjobは`pending`へ戻して試行回数を0から再開します。これらのAPIは`ENVIRONMENT`が`development` / `local` / `preview` / `test`のいずれかへ明示設定され、roleが`admin`の場合だけ有効とし、それ以外では404を返します。通常のBrain Item変更で新しいrevisionの補正jobが作られた場合は新しいjobとして同期します。原因が未解消のまま自動復帰させず、恒久障害でalarmとQueueのループを再開しないことを優先します。操作ログはIDや本文を含めず操作種別、結果、件数だけを90日保持します。
...
Brain Itemの`goal`を自動的に継続対象にはしません。AIチャットが本人の発言に基づく非推定のGoalと次の一歩を提案し、本人が明示的に合意した時だけ`goal_follow_ups`へ保存します。合意後の一覧、訂正、完了、停止、再開はWebで扱います。Liteは同時に1件、Fullは複数件を保持でき、会話時は現在の話題に関係する有効な1件だけをContextへ追加します。根拠Source Recordを伴わないGoalは利用しません。

完了、停止、次の一歩の訂正は本人操作で状態遷移させ、active以外は会話へ戻しません。`stopped`から`active`への遷移だけを明示再開として許可し、`completed`は同じrowで再開しません。同じ内容に再度取り組む場合も新しいBrain Itemと合意を必要とします。未実行、停止、訂正を人物や意欲の評価へ変換しない規則をpromptへ固定します。合意履歴はAccountDataに閉じ、Planを下げた後も読み取りを維持しますが、新しい合意や会話横断利用は開始しません。

Plan変更によって新Planの同時active上限を超える場合は、変更開始前に`agreed_at`が古い順で自動停止予定を提示し、本人の了承がなければPlan変更を開始しません。期間末変更では申込時に対象を提示し、新Planが有効になった後に同じ順序で上限内へ収束させます。自動停止も通常の`stopped`として扱い、後に上限へ空きができた場合だけ本人が再開できます。

### 4.16 本人確認済みセルフケアContext

セルフケア相談へ使う本人情報は、本人が「役立った」「合わなかった」「最近の状態」として確認した非推定Brain Itemだけを`self_care_confirmations`で管理します。Liteは各種の最新1件、Fullは複数の対処履歴と30日以内の状態を利用し、Freeは一般案だけに限定します。撤回済み、期限切れ、根拠が削除された情報は利用しません。

現在の発言を過去情報より優先し、相談意図がある通常会話にだけContextを追加します。決定的な安全分類はEntitlementと個別化Contextの読み取りより先に実行し、危機表現では全Plan共通で個別化、上限消費、通常生成を止めて安全案内へ切り替えます。合わなかった方法は再提案せず、確認済み情報がない場合は一般案を本人固有の方法として表示しません。
...
実装では現在Turnのuser発言と、一意に照合できた関係人物の検索hintを合わせて最大10,000文字の`RETRIEVAL_QUERY`としてembeddingします。検索hintは相性共有の表示名、Relationship Category、現在発言の関係語だけに限定し、最大1,000文字とします。上限調整では検索hintより現在Turnのuser発言末尾を優先して残します。相対日付はWorkerの処理時刻ではなく、user messageごとのSource Record受信時刻を基準に絶対表現へ変換します。`owner_scope` filter適用後の上位10件を候補にし、cosine scoreが0.7未満の候補は関連なしとして除外します。AccountDataはvector ID対応表から、本人所有、active、未削除、有効期間内、activeなAccess Labelありを再検証し、類似度順の最大5件を返します。支持Evidence原文は未削除の本人Source Recordだけを関連度順のItemから新しい順に選び、Context全体で最大3件にします。Gemini入力時にstatementは1件2,000文字、Evidenceは1件1,000文字を上限にします。検索には2秒の独立timeoutを設け、Vectorize・embedding・再認可の失敗時と同様に、本文を含まないdegraded logを残して記憶なしの通常返信を継続します。検索timeoutは生成全体の90秒deadlineをabortしません。
...
| `self_harm_possible` | 安全確認と、運営が確認した日本国内の公的な支援先を優先 | 記録ゴールと候補生成を停止 |
| `imminent_danger` | 119・110、医療機関、信頼できる人への即時連絡を案内 | 通常生成と候補生成を停止 |
| `abuse_or_violence` | 本人の安全と選択を優先 | 危険な手順を出さない |

危機分類を理由に発言を無言で拒否しません。provider filterが本文を返さない場合も審査済み定型文を返します。初期提供地域は日本に限定し、119・110は`imminent_danger`だけで案内します。相談先と電話番号はモデル出力へ委ねず、厚生労働省など公的機関の公開情報から運営が確認した定型情報だけをアプリケーションで付加します。安全経路から運営者や第三者への自動連絡は行いません。

アプリケーションが付加する確認済みの案内先は、`self_harm_possible`では厚生労働省「まもろうよ こころ」の[相談窓口一覧](https://www.mhlw.go.jp/mamorouyokokoro/soudan/)、`abuse_or_violence`では政府の「あなたはひとりじゃない」[制度・相談窓口一覧](https://notalone-cao.go.jp/support/)とします。`imminent_danger`ではURLの案内より119・110、医療機関、信頼できる人への即時連絡を優先します。

安全性とAI応答品質には数値の合格閾値を置かず、Previewで診断だけ、チャットだけ、両方を含む代表入力と安全経路をサービス責任者が確認し、承認した記録を公開条件とします。
