...
自動のAccount再接続は行いません。初期提供では運営者による例外復旧も行わず、問い合わせを受けてもAccountの再接続、Identity移動、Account復旧としての本人データ開示を実施しません。請求情報だけでは、日記や診断を持つAccountの所有を証明できないためです。

有料契約の停止だけを希望する場合は、Stripeが提供する本人向けの契約管理・支払異議申立て経路を案内します。請求上の本人確認を行う場合も、その確認をアプリAccountの復旧や本人データの開示へ流用しません。
