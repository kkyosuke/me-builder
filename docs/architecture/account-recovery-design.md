...
認証済み本人は設定画面で復旧コードを発行できます。新しいコードを発行すると未使用の旧コードを失効させ、コードの保存が本人の責任であること、有効期限、再表示できないことを明示します。Accountの削除、停止、乗っ取り調査中は発行しません。
...
コード照合の失敗応答は、Accountやコードの存在を推測できない同一内容にします。IPと新Identity単位で試行を制限し、一定回数の失敗後は短時間ロックします。成功、失敗、ロック、コード発行・失効を監査しますが、復旧コード、LINE token、Stripe識別子は記録しません。
