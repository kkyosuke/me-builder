...
自動のAccount再接続は行いません。初期提供では運営者による例外復旧も行わず、問い合わせを受けてもAccountの再接続、Identity移動、Accountへのアクセス付与を実施しません。請求情報だけでは、日記や診断を持つAccountの所有を証明できないためです。プライバシーや請求に関する問い合わせは受け付けますが、問い合わせで得た情報を復旧コードの代わりにはしません。

有料契約の期間末解約だけを希望する場合は、Customer Portalを案内します。Customer Portalを利用できない場合は、公開した問い合わせ窓口で請求上の本人確認を行い、Account復旧と分離して期間末解約を受け付けます。この確認をアプリAccountの復旧、Identity移動、Accountへのアクセス付与へ流用しません。

将来、運営者による例外復旧を提供する場合は、別途承認された複数要素の確認手順、二者承認、待機期間、対象Accountへの通知を設計し、この原則を改定してから提供します。
