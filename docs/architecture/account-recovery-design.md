...
復旧コードは十分な乱数から作り、平文は発行時に一度だけ返します。共有D1にはversion付きのsalted hash、対象Account、有効期限、使用・失効時刻だけを保存します。ログ、分析、ブラウザtelemetry、エラー報告、問い合わせ記録へ平文を残しません。
...
自動のAccount再接続は行いません。本人データの開示やIdentity移動は保留し、問い合わせ窓口へ誘導します。メールアドレス、請求情報、Stripe Customerの所有だけを根拠に、Accountの再接続、Identity移動、本人データの開示を行いません。

有料契約の停止だけを希望する場合は、Stripeのコード不要Customer Portalへ案内します。利用者は購入時に登録したメールアドレスとStripeが送るワンタイムパスコードで入り、Accountを復旧せずに期間末解約、支払方法の更新、請求書の確認を行えます。Portalへ入れない場合は[公開中の問い合わせ窓口](../product/service-site-design.md#73-お問い合わせ)で受け付け、請求上の本人確認後に期間末解約だけを運営経路から行います。この手続きでもアプリAccountを復旧した扱いにはせず、本人データを開示しません。

初期提供では運営者による例外的なAccount復旧を行いません。将来提供する場合は、別途承認された複数要素の確認手順、二者承認、待機期間、対象Accountへの通知を設計してからこの原則を改定します。
