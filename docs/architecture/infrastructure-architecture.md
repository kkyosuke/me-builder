...
| **外部LLMゲートウェイ** | **Cloudflare AI Gateway** | Queue Worker から Vertex AI Express Mode の Gemini を呼び出す際の統一経路。Gateway の認証、可観測性、利用量管理を担い、Vertex AI API key をクライアントへ公開しない。 |
...
5. **外部LLMの呼び出し**
   - Vertex AI Express Mode の Gemini を利用する処理は、Queue Worker から Cloudflare AI Gateway の Google Vertex AI provider URL を経由して呼び出します。
   - `@google/genai`は`vertexai: true`とAPI version `v1`で初期化し、プロジェクトやロケーションの指定を要しないExpress ModeのAPI key認証を使います。
   - Vertex AI API key (`GOOGLE_VERTEX_AI_API_KEY`) と AI Gateway token は Worker の Secret として保持し、Web UI、APIレスポンス、ログへ露出させません。
