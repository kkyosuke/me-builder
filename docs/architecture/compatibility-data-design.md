...
招待発行前の確認と発行commandは、表示名と共有表示全体から決定的に計算するバージョン付きの不透明なpreview tokenで結びます。発行時はAccountDataの現在状態から表示内容とtokenを再計算し、クライアントが確認時に受け取ったtokenと一致する場合だけ、その時点のテーマ別`result_fingerprint`を保存します。token自体をテーマ別`result_fingerprint`として保存または公開せず、不一致時は古い確認内容で発行せず再確認を求めます。
