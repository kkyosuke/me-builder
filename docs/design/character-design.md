...
- サービスビジュアル（アイコン・バナー）の構図と、切り抜き時の前提
...
うつしとミラが並ぶ1枚絵を、アイコン用とバナー用の2つのアスペクト比で持ちます。両方に共通する要素は次のとおりです。

- うつしを左手前に大きく置き、右側にミラを浮かべる。うつしは手を上げた挨拶の姿勢
- 背景はパステルの虹色グラデーションで、花と光（キラキラ）のモチーフを散らす

### アイコン

正方形（1254×1254px）。ミラは[素体](#4-ミラ)の姿で、うつしのすぐ右隣に寄せます。

円形や角丸に切り抜かれる用途では、四隅の花と光のモチーフが欠けることを前提とします。

### バナー

横長（1774×887px、2:1）。ミラは[魔法書を持つバージョン](#別バージョン)で、うつしから離れた右側に配置します。

横幅が切り詰められる用途では、左右の余白から先に欠けることを前提とします。うつしとミラの間に空きがあるため、この余白を文字の載せ場所として使えます。
...
複数のキャラクターが登場するアセットや、キャラクターに属さないアセットは `assets/` 直下へ置き、用途を表す名前を付けます（例: `app-icon.png`、`banner.png`）。

## 7. アセットを追加するときの手順

1. `docs/assets/` へ[命名規則](#6-アセットの置き場所と命名規則)に沿って配置する
2. 該当キャラクターの節、または[サービスビジュアル](#5-サービスビジュアル)の節のアセット表へ1行追加する
