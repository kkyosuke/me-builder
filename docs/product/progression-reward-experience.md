...
この文書は、物理データモデル、API、イベントスキーマ、通知頻度、報酬アセットの制作仕様を所有しません。利用者向けの名称、加点値、アンロック内容は初期検証で使う案であり、検証結果を基に版を更新します。うつしレベルの式は[§4.2](#42-レベル式)を採用します。
...
2. **わたしのかけら**: Brain Itemを表す単位。これまで集めた個数と、現在有効な個数を分けて表示する
...
| 線形増加 | レベルごとに一定量ずつ増える | 累積必要値は二次式になり、序盤と長期を調整しやすい | 採用 |
| 指数増加 | 一定倍率で増える | 長期では必要値が急増し、実質的な上限になりやすい | 採らない |

累積成長値`G`でレベル`L`へ到達する閾値には、次の二次式を採用します。条件を満たす最大の`L`が現在レベルです。
...
うつしレベルは「これまでに理解が育った歩み」として維持します。「わたしのかけら」は、次の2つの数を区別して表示します。

| 表示 | 数えるもの | 増減 |
| --- | --- | --- |
| 集めたかけら | これまで新しく生成されたBrain Itemの累計 | 新規Itemで増え、期限切れやRevisionでは減らない |
| 有効なかけら | 現在`active`で利用可能なBrain Item | 新規Itemで増え、期限切れ、置き換え、無効化で減る |

Evidence追加は既存Itemを深める出来事なので、うつしレベルの成長値には反映しますが、集めたかけら数は増やしません。時間によるRevisionでは、新版を集めたかけらへ1個加え、旧版も「以前の自分」として履歴に残します。

```text
これまでの歩み: うつし Lv.12       ← 下がらない
わたしのかけら: 集めた 58個       ← 過去の変化も含む
                  有効 48個         ← 現在利用できるItem
分類の広がり:     6種類
```

`Current State`など期限を持つItemは、有効期限が切れるか新しいItemに置き換えられた時点で有効なかけらから除き、集めたかけらと「以前の自分」の履歴には残します。期限を推測できないItemを画面側だけで失効させません。失効しても、集めたかけら数とうつしレベルは下げません。

削除後に以前と同じ結果を表示し続けたり、レベル維持を理由に削除済みのstatement、分類、Evidenceを利用したりしてはいけません。削除したかけらは履歴一覧にも内容を表示せず、集めたかけらの累計とうつしレベルに、元の命題を復元できない集計事実だけを残します。
...
式や加点値を更新しても、確定済みの出来事、累積値、レベルは再計算しません。新しい式は変更後に確定する出来事だけへ適用し、過去の獲得結果を後から変えません。
...
| レベルアップ | レベル数値と次のレベルまでの値を更新 | 新しいレベルを表示する |
| 10レベルごとの節目 | 節目へ到達したことを静的に表示 | 成長カードを保存できる |

レベル数値と進捗バーで成長を十分に説明できるため、うつし本体の造形変更、花・光のリング、レベルアップ演出は初期導入に含めません。画面を開いたときに数値が更新されていればよく、常時動くエフェクトやスキップ操作を増やしません。

成長カードには、到達レベル、到達日、前回の節目から増えたかけら数だけを載せます。分類名、Brain Itemのstatement、Evidence本文、内部識別子やmetadataは載せません。背景色やカード枠などの選択肢は最初の数回の節目で有限個をアンロックできますが、カタログを取り切った後も10レベルごとに異なる数値と日付の成長カードを得られます。期間限定、ランダム排出、未取得へ戻る報酬は設けません。

分類の広がりはレベルの加点係数にせず、集めたかけら・有効なかけらの隣に別表示します。量と幅を混ぜないことで、診断中心の人とチャット中心の人のどちらかを不利にしません。
...
| 回答・発言を削除する | 有効なかけら数と利用結果を更新 | 獲得済みレベルを下げない |
...
「今のわたし」より上に大きなゲーム画面を追加せず、ヘッダー付近にうつしレベル、次のレベルまでの成長値、集めたかけら数と有効なかけら数を表示します。

```text
うつし Lv.12
████████░░  次のレベルまで、あと8
わたしのかけら 集めた58個・有効48個  >
6つの分類
```
...
診断完了時は、回答結果を報酬より先に表示します。Brainへの反映が完了した後、レベル、成長値、かけら数の変化を静的に表示します。
...
1. 既存利用者の開始値を[§4.5](#45-既存利用者の開始値と式の更新)で計算し、うつしレベル、次のレベルまでの進捗、集めたかけら数、有効なかけら数を表示する
2. 新規Item、異なるEvidence、時間によるRevisionだけが成長値へ反映され、再配送、同じ発言、単なる訂正では増えないことを確認する
3. 10レベルごとの成長カードを表示する
...
| 体験群 | うつしレベル、集めたかけら、有効なかけら、反映フィードバックを表示する |
