...
かがみ（リポジトリ上の開発名: me-builder）は、誰でも自分自身の価値観、経験、好み、考え方をデータとして蓄積し、その人らしく応答する「自分の分身（Me Agent）」を作成できるサービスです。
...
質問の分類や公開範囲、回答の履歴など、詳細な管理方法はデータ構想とあわせて今後検討します。公開範囲は変更可能にし、入力後の訂正・削除可否は入力種別ごとに定めます。受理済みの診断回答は訂正・個別削除できません。

Phase 1で実際に提供する入力の形式は[§4](#4-想定する利用体験)で確定しています。

### 3.2 MCPでエージェントへ提供する

Phase 2以降で蓄積した情報をMCPサーバーとして公開し、本人が許可したAIエージェントから利用できるようにします。MCPは初期リリースには含めません。
...
LINEとWebをまたぐ入口、Webの主ナビゲーション、診断・本人向け情報・LINEのAI相談・管理者画面の接続は[全体画面遷移設計](screen-navigation.md)を正とします。
...
- **iOS・Androidネイティブアプリを起点にする**: インストール、store審査、OS別保守が増える一方、必要な体験をLINEとWebで提供できるため採用しない

#### 目的に合わせた初回ガイド（検討候補）

初めて利用する人が目的を選び、最初の診断から「わたしのまとめ」を作るまで進めるガイドを検討します。目的の例は「自分を知りたい」「人との違いを知りたい」です。選んだ目的に応じて、公開済みの診断から最初に取り組む候補を案内し、既存の診断・まとめ画面へ接続します。提供時期は未決定です。

最小構成は、任意の目的選択、最初の診断への案内、まとめ作成への案内です。ガイドを閉じて通常の機能を使うこともできるようにし、相性共有への同意や相手の招待をガイド完了の条件にはしません。診断回答は[Phase 1 診断体験設計](../diagnosis/diagnosis-experience.md)、まとめ生成は[わたしのまとめ仕様](profile-summary-experience.md#23-新しい版の生成)の状態と条件に従い、生成中・生成不可の場合も現在の状態と次にできることを案内する候補にします。
...
「初回の3分ガイド」は体験を短くするための目安であり、診断と非同期生成を含む完了時間の保証にはしません。検証では、利用開始から最初の診断完了・まとめ作成へ進んだ割合と所要時間を確認し、実際の結果から案内する診断と手順を調整します。

着手時には、目的と診断の対応、中断・再訪時の案内、既に回答やまとめがある場合の省略方法を具体化します。招待や診断への直接リンクで来た人の要求画面を一律のガイドで遮らず、認証・規約同意・初期表示の扱いは[全体画面遷移設計](screen-navigation.md)と[Web初回起動体験設計](web-startup-experience.md)を正とします。
...
**確定**: LINE内からの主導線はLIFF（LINE Front-end Framework）でWebを開く形とします。あわせて、外部ブラウザから独立して開けるWebも維持し、既存Accountへプロフィール画面から追加したGoogleログインで認証します。Googleログインだけでは新規Accountを作りません。

| 導線 | 開き方 | 本人性の保証 |
| --- | --- | --- |
| LINE内（主導線） | トークやリッチメニューのリンクをLIFFで開く | LIFFが引き継ぐLINEのログイン状態 |
| 外部ブラウザ | URLを直接開く | 既存Accountへ追加済みのGoogleログイン |

```mermaid
flowchart TD
    K[Webを開くリンク] --> J{どこから開いたか}
    J -->|LINE内| F[LIFFで表示する<br/>LINEのログイン状態を引き継ぐ]
    J -->|外部ブラウザ| B[追加済みのGoogleログインで認証する]
    F --> S[同じWeb画面]
    B --> S
```
...
- GoogleログインはLIFFで本人確認済みの既存Accountからだけ追加し、email一致でAccountを推測しないため、外部ブラウザの入口を増やしてもAccountの起点はLINEに保てる
...
- **LIFFを採らず外部ブラウザだけで開く**: 実装は最小だが、LINEを起点とする1日1回の反復操作に対してログインの摩擦が大きい
...
| --- | --- | --- |
| 日記チャット | 担当する | 提供しない |
| AI相談 | 担当する | LINEの相談への入口だけを提供する |
| 日々の声かけ | 担当する | 提供しない |
| 診断の回答・結果閲覧 | Webへのリンクを配信する | 担当する |
| MCP連携設定 | 提供しない | 担当する |
| プロフィール・課金・データ管理 | Webへ誘導する | 担当する |
...
**確定**: Phase 1では、日記の原本を本人が入力したまま`private`なSource Recordとして先に保存し、その後にAIを使って本人が明言した内容をBrain Itemへ変換して蓄積します。診断の回答も原本は入力どおり保存し、Brain Itemへの変換にはAIではなく版付きの決定的な計算規則を使います。共通の生成規則と入力元ごとの差分は[Brain Item生成設計](../domain/brain/brain-item-generation-design.md)を正とします。

AIが生成したBrain Itemは、原本と混同せず、根拠になったSource RecordとAI由来であることを保持し、既定のAccess Labelを`unclassified`とします。本人が明言していない内容の推定、AIによるAccess Labelの拡張、外部提供可否の自動変更はPhase 1に含めません。

根拠:

- 原本を先に保存することで、AIの出力品質にかかわらず本人の入力を失わず、後から根拠をたどれる
- 日記の自由記述から本人が明言した情報をBrain Itemとして蓄積し、回答を続けたくなる体験と、蓄積した情報が本人に役立つかを同じ段階で検証できる
- Source Recordへ`private`を付けてからAIへ渡し、生成物を`unclassified`にすることで、[ラベル判定前の本文をLLMや外部検索サービスへ送らない](../domain/brain/brain-access-label-design.md#1-結論)という制約と、外部提供を本人が制御する原則を両立できる

## 5. アカウントと本人識別

LINEとWebは、同じAccountと分身へアクセスする利用チャネルとして扱います。iOS・Androidネイティブアプリは提供しません。
...
**確定**: Phase 1のログイン手段はLINEだけとします。検証済みのLIFF／LINE Loginで初めてWebへアクセスした時点、または署名検証済みのLINE Webhookで友だち追加・メッセージを初めて受け付けた時点をAccount作成の起点とします。Webの本人性はLINE側のログイン状態で保証し、LINE公式アカウントを友だち追加していなくてもWeb機能を利用できます。導線ごとの開き方は[§4](#4-想定する利用体験)で確定しています。

Webから先に利用を始めたAccountは、LINE Loginのidentityだけを持ちます。友だち追加またはメッセージのWebhookを受け付けた後にMessaging APIのidentityを同じAccountへ追加し、それまではLINEからの日記入力、通知、日々の声かけを利用できません。

```mermaid
flowchart TD
    A{最初の入口} -->|LIFF / LINE Login| W[AccountとLINE Login identityを作成]
    A -->|友だち追加 / メッセージ| L[Accountと両方のLINE identityを作成]
    W --> F{後から友だち追加<br/>またはメッセージ}
    F -->|受信前| WO[Web機能のみ利用]
    F -->|受信後| C[Messaging API identityを同じAccountへ追加]
    L --> B[Web機能とLINE機能を利用]
    C --> B
```

**確定**: 有料契約中に旧Identityで発行・保管した一回限りの復旧コードがある場合は、新しいLINE Identityで既存Accountへ再接続し、同じ本人データとPlanを継続できます。事前コードがない場合、初期提供では自動・有人のどちらでもAccountを再接続せず、問い合わせだけでIdentityを移動したり、Accountへのアクセスを付与したりしません。Customer Portalを利用できない場合の期間末解約は、Account復旧と分離して受け付けます。認可条件と復旧不能時の境界は[Account復旧設計](../architecture/account-recovery-design.md)を正とします。
...
- Webを先に開いた利用者にも、検証済みのLINE Login identityをAccount作成の根拠として使える。公式アカウントとの接続はMessaging APIのidentityの有無で区別できる
- LINEだけを通常のログイン手段としたまま、旧Identityで事前発行した資格情報と新Identityの認証を組み合わせることで、別のログインproviderを登録時に必須化せずAccount喪失へ備えられる
- 復旧時にAccountを作り直さずIdentityだけを再接続するため、本人データとPlanの所有先を変えずに継続できる
...
2つ目のログイン手段としてGoogleを提供します。既存Accountのプロフィール画面からだけ追加でき、Googleだけの新規Account作成とemailによるAccount統合は行いません。Sign in with Apple、メールアドレス、パスキーなど、Google以外の手段はPhase 2以降に検討します。

LIFFとGoogleログインを各機能から分離し、同じAccountとアプリケーションセッションへ収束させる技術境界と製品接続条件は[Web認証・アプリケーションセッション設計](../architecture/web-authentication-design.md)を正とします。
...
- MCP接続先ごとの権限とアクセス履歴
- 外部連携での特徴利用方法、および物理削除の方法（取得境界は[Source Recordのライフサイクル設計](../domain/source/source-record-lifecycle-design.md)で確定済み）
...
- ユーザーが回答を訂正・削除でき、APIから本人の派生特徴だけを取得できる
- AI学習への利用はサービス利用と分離して同意を得る
- 第三者の顔や声を含むメディアに配慮する
- サービス利用に年齢制限を設けず、生年月日を収集しない。将来18歳未満が有料プランを購入する場合だけ保護者同意を確認する
...
無料・有料の境界、初期価格、AI利用上限、トライアル、解約後の扱いは[サブスクリプション・料金プラン設計](subscription-plan-design.md)を正とします。同文書にある将来の有料機能は、以下のMVPとロードマップ上の提供時期を変更せず、実装済みであることも意味しません。
...
- テキストと画像を含む質問定義
- 選択式の診断回答
- 保存済み診断回答の確認。受理済み回答の訂正・個別削除は提供しない
- 開発環境だけの日記確認・訂正・削除と、本文を含まないBrain特徴JSONの書き出し
- ユーザー向けプロフィール要約（[わたしのまとめ仕様](profile-summary-experience.md)を正とする）
...
Phase 1で初期リリースのMVPを構築します。Phase 2でMCPによる外部接続を追加し、Phase 3以降で付加機能へ展開します。

### Phase 1: 回答を集める

回答を続けたくなる体験と、蓄積した情報が本人に役立つかを検証します。入力体験は[§4](#4-想定する利用体験)、ログイン手段は[§5](#5-アカウントと本人識別)で確定しています。日記は原本のSource Recordを保存した後、AIで本人が明言した内容をBrain Itemへ変換して蓄積します。診断回答のBrain Itemは版付きの決定的な計算規則で生成します。Source RecordとBrain Itemの関係は[ドメイン設計 §6](../domain/domain-design.md#6-ドメイン間の関係)、生成処理は[Brain Item生成設計](../domain/brain/brain-item-generation-design.md)を正とします。

### Phase 2: 外部と接続する（初期リリース後）
...
以下は、既存機能を日常の判断や実践へつなげるための検討順です。利用データによる効果検証前の優先順位であり、リリース順、提供時期、料金プランへの追加を確定するものではありません。各案の体験と確認事項はリンク先だけで定義します。

| 検討順 | 候補・追跡先 |
| --- | --- |
| 1 | [迷いごと比較ノート](diary-chat-experience.md#迷いごと比較ノート検討候補) |
| 2 | [ふたりの「試してみること」](compatibility-experience.md#545-ふたりの試してみること検討候補) |
| 3 | [自分の記憶帳](profile-summary-experience.md#29-自分の記憶帳検討候補) |
| 4 | [目的に合わせた初回ガイド](#目的に合わせた初回ガイド検討候補) |
| 5 | [写真日記の公開](../development/diary-remaining-tasks.md#21-日記の写真添付を公開する) |

最初の試作候補は迷いごと比較ノートとします。写真日記は設計済みの保存段階を公開する残作業として扱い、その公開準備をこの表の他の候補の実装完了に依存させません。

## 12. 今後決めること

Phase 1に必要な決定（対応チャネルと入力形式、チャネルの役割分担、ログイン手段と復旧方針、質問の作成主体と版管理、AIを使う範囲）は[§4](#4-想定する利用体験)と[§5](#5-アカウントと本人識別)へ移しました。
