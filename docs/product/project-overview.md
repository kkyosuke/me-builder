...
**確定**: LINEアカウント自体を失った場合のアカウント復旧は、Phase 1では保証しません。復旧できないことを利用開始時に明示し、蓄積したデータのエクスポートはV2で提供します。
...
- 復旧しない前提は「データの主導権はユーザー本人が持つ」という原則（[§13](#13-現時点のプロダクト原則)）に対して弱いため、V2のエクスポートで補う
