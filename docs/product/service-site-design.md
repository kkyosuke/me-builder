...
初期公開は、トップページを中心とした小さな構成にします。機能ごとの紹介ページや読み物を先に増やさず、トップページ、利用規約、プライバシーポリシー、問い合わせの4領域を揃えます。有料Planを公開する段階で、認証不要の「特定商取引法に基づく表記」を同じ公開サイトへ追加します。
...
| `/commercial-transactions` | 有料Planの料金、支払、trial、自動更新、解約、返金と事業者情報の開示請求方法を示す | 有料公開時に必須 |
...
- LINE Accountを利用できなくなった場合、本人向け画面に表示された復旧コードを事前に保存したAccountだけが復旧の対象であり、コードがなければ自動・有人のどちらでも同じAccountへの再接続や保存済みデータへの再アクセスを可能にしない
...
Free限定公開中の問い合わせ種別を少なくとも「サービスについて」「不具合」「データとプライバシー」「権利侵害」に分けます。有料Planを一般提供する段階で「契約・請求」を追加します。送信前に、返信に必要な連絡先と問い合わせ本文がどのように扱われるかを示します。

日記や診断回答の原文、復旧コード、認証token、LINE user IDなどの機微情報を入力するよう求めません。Account内データの変更は問い合わせでは受け付けず、本人向け画面だけで行います。問い合わせで得た情報を復旧コードの代わりにして、同じAccountへ再接続したり保存済みデータへのアクセスを付与したりしません。有料Planを一般提供する段階では、Customer Portalを利用できない有料契約の期間末解約をAccount復旧と分離して受け付けることを示します。
...
- 利用規約、プライバシーポリシー、問い合わせは直接リンクから到達可能にする
- トップページは検索対象にする
- 法務本文と問い合わせ運用は、対応する公開前レビューが完了するまで`noindex,nofollow`とし、承認後だけ検索対象へ切り替える
- 特定商取引法に基づく表記は、有料Planの一般提供と商取引条件の承認が両方完了した後だけ公開導線と検索対象へ加える
...
- 認証していない訪問者がトップ、利用規約、プライバシーポリシー、問い合わせを閲覧できる。有料Planの一般提供前に特定商取引法に基づく表記へ直接到達した場合は、現在は有料Planを公開していないことだけを表示する
...
- 利用規約、プライバシーポリシー、問い合わせへフッターから移動できる。特定商取引法に基づく表記へのリンクは有料Planの一般提供後だけ表示する
