...
| 必須 | 料金 | 初期プランと価格は設計済みだが、提供開始時期、決済方法、返金条件が未確定 | 公開時点の提供機能、支払条件、更新、トライアル、解約を責任者が確定する |
| 必須 | 掲載機能の提供状態 | 設計済みの機能と初期リリースのPhaseが一致しない箇所がある | 公開時点の各機能を「利用できます」「準備中です」「構想中です」に分類する |
| 必須 | CTAの遷移先 | 公開サイトはLINE公式アカウントの友だち追加へ接続し、`/app`と既存パス、LIFF deep linkは既存利用者向けに維持する | 実環境で友だち追加、リッチメニュー、外部ブラウザ、LINE内ブラウザから本人確認後の復帰を確認し、遷移失敗時の案内を用意する |
