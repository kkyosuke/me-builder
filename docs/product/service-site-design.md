...
初期の問い合わせ窓口は`support@kagami.kyosuke.dev`とし、LINEトークを問い合わせ窓口に使いません。一般公開前にこのアドレスが実際に受信でき、継続して返信できることを確認します。
...
販売業者は個人事業者の`河村京介`と表示します。住所と電話番号は通常の公開画面へ掲載せず、「請求があった場合は、購入の判断前に十分確認できるよう遅滞なく電子メールで提供する」旨と請求先の`support@kagami.kyosuke.dev`を表示します。運営者は正確な住所と確実に連絡できる電話番号を公開リポジトリ外で管理し、請求時に同じ窓口から提供できる状態を維持します。
