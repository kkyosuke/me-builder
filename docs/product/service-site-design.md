...
- LINE Accountを利用できなくなった場合、事前に復旧コードを保存した有料契約Accountだけが復旧の対象であり、コードがなければ自動・有人のどちらでもAccountと本人データを復旧しない
...
日記や診断回答の原文、認証token、LINE user IDなどの機微情報を入力するよう求めません。Account内データの変更は問い合わせでは受け付けず、本人向け画面だけで行います。
