...
初期公開は、トップページを中心とした小さな構成にします。機能ごとの紹介ページや読み物を先に増やさず、トップページ、利用規約、プライバシーポリシー、特定商取引法に基づく表記、問い合わせの5領域を揃えます。
...
| `/commercial-transactions` | 販売業者、価格、支払、提供、トライアル、更新、解約、返金の条件を示す | 有料提供前に必須 |
| `/contact` | 利用前の質問、データ削除、権利侵害、障害の連絡先を示す | 必須 |
| `/app` | LINEによる本人確認後、要求されたアプリ画面へ進む共通入口 | 必須 |

利用規約、プライバシーポリシー、特定商取引法に基づく表記は、トップページの一部だけに埋め込まず、個別URLで直接参照できるようにします。Webアプリの同意画面、プロフィール、LINEの案内からも必要な公開文書へ到達できるようにします。
...
- 中央またはメニュー: 「できること」「使い方」「安心して使う」「料金プラン」「よくある質問」。初期公開ではトップ内の該当セクションへ移動する
- 右: LINE公式の「友だち追加」ボタン
...
- LINE Accountを利用できなくなった場合、事前に復旧コードを保存した有料契約Accountだけが自動復旧の対象であり、コードがなければ自動再接続できない
...
Free、Lite、Full、ファミリーパックの月額・年額と、利用できる主な機能をトップページ内で比較できるようにします。価格と利用権限は[サブスクリプション・料金プラン設計](subscription-plan-design.md)だけを正とし、公開サイト固有の価格や上限を作りません。

- Freeを含む4プランを同じ比較対象として表示する
- 有料Planは利用者が支払う月額・年額の総額と初回トライアルを表示し、税や決済手数料を別途加算しない
- 日記、診断、「わたしのまとめ」、基本の相性シートは比較表へ重複表示せず、全Plan共通機能として表の前で案内する。「わたしのまとめ」は入力または生成形式の変更と前回生成から7日経過の両方が必要であり、月ごとの回数上限を設けないことをPlan差ではなく注記で案内する
- AI返信は正常に届けたAIの回答を1回として数えることを注記し、Freeは月60回（1日2回利用の約30日分）と案内する
- Plan差はAI返信、週次振り返り、過去との比較、行動フォロー、セルフケア、Account数で示す。具体的な提供内容が未実装の将来構想は公開料金表へ載せない
- モバイルでは各Planの概要を先に読め、詳細比較表は横スクロールできるようにする
...
- 無料で使えますか
- Free、Lite、Full、ファミリーパックは何が違いますか
...
プランの価格と利用権限は[サブスクリプション・料金プラン設計](subscription-plan-design.md)を正とし、公開時点で利用できる機能だけを回答します。決済方法、返金条件、削除方法、問い合わせ先など未確定の回答を推測で掲載しません。回答できない項目がある状態は、公開前の未決事項として扱います。
...
初期の問い合わせ窓口は株式会社AccelHackが管理する`kagami@accelhack.com`とし、LINEトークを問い合わせ窓口に使いません。一般公開前にこのアドレスが実際に受信でき、継続して返信できることを確認します。メールアドレスの作成・転送設定は、このリポジトリのPulumi管理対象に含めません。

問い合わせ種別を少なくとも「サービスについて」「不具合」「課金」「データとプライバシー」「権利侵害」に分けます。送信前に、返信に必要な連絡先と問い合わせ本文がどのように扱われるかを示します。

日記や診断回答の原文、認証token、LINE user IDなどの機微情報を入力するよう求めません。データ削除など本人確認が必要な手続きは、公開フォームだけで完了させず、安全な確認手段へ案内します。

### 7.4 特定商取引法に基づく表記

販売業者は法人の`株式会社AccelHack`と表示します。住所と電話番号は通常の公開画面へ掲載せず、「請求があった場合は、購入の判断前に十分確認できるよう遅滞なく電子メールで提供する」旨と請求先の`kagami@accelhack.com`を表示します。株式会社AccelHackは正確な住所と確実に連絡できる電話番号を公開リポジトリ外で管理し、請求時に同じ窓口から提供できる状態を維持します。

この省略方法は、消費者からの請求により広告表示事項を遅滞なく提供できる場合に氏名・住所・電話番号を省略できるとする[消費者庁の通信販売広告Q&A](https://www.no-trouble.caa.go.jp/qa/advertising.html)に従います。メールが未開通、返信担当が不在、正確な情報を提供できない状態では有料Planの申込みを受け付けません。

このページには[サブスクリプション・料金プラン設計](subscription-plan-design.md)を参照して、支払総額、支払方法と時期、提供開始、トライアル、自動更新、契約期間、解約、返金条件を掲載します。価格や条件の別定義をこの文書へ作りません。
...
- 利用規約、プライバシーポリシー、特定商取引法に基づく表記、問い合わせを検索と直接リンクから到達可能にする
...
- トップページの閲覧
- 主CTAの選択
- 利用規約、プライバシーポリシー、特定商取引法に基づく表記、問い合わせへの遷移
- CTA遷移の失敗
...
- 認証していない訪問者がトップ、利用規約、プライバシーポリシー、特定商取引法に基づく表記、問い合わせを閲覧できる
...
- Freeを含む4プランの税込価格と利用できる主な機能を比較できる
- 本人の入力とAIの出力、本人向け利用と他者共有を混同していない
- 医療的・心理学的な診断や専門的助言でないことを、FAQまたは安全性の説明から確認できる
- 利用規約、プライバシーポリシー、特定商取引法に基づく表記、問い合わせへ、トップとフッターから移動できる
