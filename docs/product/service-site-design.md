...
初期の問い合わせ窓口は株式会社AccelHackが管理する`kagami@accelhack.com`とし、LINEトークを問い合わせ窓口に使いません。一般公開前にこのアドレスが実際に受信でき、継続して返信できることを確認します。
...
販売業者は法人の`株式会社AccelHack`、適格請求書発行事業者の登録番号は`T8010001231255`と表示します。住所と電話番号は通常の公開画面へ掲載せず、「請求があった場合は、購入の判断前に十分確認できるよう遅滞なく電子メールで提供する」旨と請求先の`kagami@accelhack.com`を表示します。株式会社AccelHackは正確な住所と確実に連絡できる電話番号を公開リポジトリ外で管理し、請求時に同じ窓口から提供できる状態を維持します。
