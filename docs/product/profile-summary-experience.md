...
本人が過去の日記を探して読み返す「自分の記憶帳」の検討案は[§2.9](#29-自分の記憶帳検討候補)で扱います。
...
- 回答や日記の追加後は再生成理由が表示され、新しい版を作るとまとめが変わり得る。開発環境で日記を訂正・削除した場合も同じ扱いとする
...
カード全体を個別の回答結果へのリンクにし、診断名と遷移先が分かる名前を支援技術へ伝えます。遷移先では、すべてのパラメータと、どの質問と選択肢が根拠になったかを確認できます。診断回答の個別の修正・削除は提供しません。
...
`development`、`local`、`preview`、`test`では、生成処理を確認するため、管理者のプロフィールに限って`DEV ONLY`導線から開く独立した画面に管理者本人のactive Brain Item一覧を表示します。共有Previewへ招待された一般テスターには導線も画面も表示しません。「わたしのまとめ」には一覧も開発用導線も表示しません。LINE内のLIFFと外部ブラウザで表示条件を変えません。statement、分類、AIまたは決定的な導出、作成日時、EvidenceのSource Record IDを表示し、0件も「追加されたBrain Itemはありません」と明示します。チェックポイントの処理状態はBrain Itemの状態ではないため、`pending` Itemとしては表示しません。

この一覧は画面を開いたときに、管理者本人のAccountデータだけを専用の開発APIから読み、最大100件を新しい順に表示します。戻る操作でプロフィールへ復帰します。Productionと一般利用者ではAPIも404を返します。否定・修正などの操作は表示しません。

### 2.9 自分の記憶帳（検討候補）

「前に似たことで悩んだとき、どうした？」から本人の日記を探し、関連する記録の日付と元の文章を確認できる体験の案です。日記を蓄積した本人が、AIの返答に使われた時以外にも自分で読み返せることを目指します。画面の提供時期は未決定です。本人データの閲覧とAI検索の利用権限は、既存の料金プラン設計に従います。

最小構成の候補は次のとおりです。

- 「わたし」から本人専用の記憶帳へ進む
- 保存済みのテキスト日記を日付順でたどって読む
- 自然な言葉で探したい出来事を入力する
- 関連するテキスト日記の一覧に、記録日時と短い抜粋を表示する
- 選んだ記録の元の文章へ進み、当時の文脈を確認する

検索結果はAIが作った人物評価や新しいまとめ版と区別します。見つからないときは過去の出来事を補わず、検索対象の期間内に該当する記録がない状態を表示する案です。検索対象外の期間に記録がないとは断定しません。

本人データの閲覧は[課金の原則](subscription-plan-design.md#31-本人のデータを人質にしない)、AIによる意味検索の対象期間は[プラン別の利用権限](subscription-plan-design.md#41-機能一覧)を正とします。日付順の閲覧から原文へ進む導線をAI検索と分け、Plan変更でAI検索の対象期間が短くなっても、保存されている本人の日記を読み返せる案にします。日付による絞り込みと検索対象期間の表示は、着手時に具体化します。

現在の開発用入力確認画面や[§2.8](#28-開発環境のbrain-item確認)の一覧を、そのまま一般公開する案ではありません。初期候補は本人のテキスト日記の検索・閲覧に絞り、Brain Itemの直接編集、診断回答の変更、外部への共有は含めません。写真の意味検索は[LINE写真日記入力設計](../architecture/photo-diary-input-design.md#2-結論)の提供段階に従い、保存段階の写真を検索対象へ広げません。

訂正・削除後の検索対象と元の記録の扱いは[Source Recordのライフサイクル設計](../domain/source/source-record-lifecycle-design.md)、情報の利用可否は[Brainのラベル・アクセス制御設計](../domain/brain/brain-access-label-design.md)を正とします。検索結果から元の記録を開く時点でも、本人が現在閲覧できる内容だけを表示することを確認項目にします。

検証では、探したい記録へ到達できた割合と、読み返した記録が現在の悩みの整理に役立ったという評価を確認します。検索・閲覧の画面、該当なしと取得失敗の区別、要約と原文の表示上の区別、利用可能期間の案内を着手時に具体化します。検索方式、API、物理データモデルはこの検討案では決めません。
...
| 開発環境での日記訂正・削除後 | 保存済みのまとめ版は変更せず、現在件数と再生成理由を更新する |
