...
最新版で再生成可能なときは、カード左上に丸い再生成ボタンを表示します。右スワイプは過去版から新しい版へ戻る操作だけに使い、最新版では再生成を開始しません。ボタンを押すと生成を要求し、「新しい版を作成中」カードを手前に表示します。最後に成功した版は後ろのスタックへ残し、作成中カードを左へスワイプするか版ボタンを選ぶと完了済みの版を引き続き閲覧できます。診断の追加、日記・記録の追加、前回生成からの時間経過のうち該当する理由を、生成カード内に短く表示します。

「新しい版を作成中」カードは、再生成ボタンを押したカードの幅と高さを引き継ぎます。生成状態へ切り替わる際にカード下の説明や導線を上下させません。

丸い再生成ボタンには「新しい版を再生成」という名前を付け、キーボードと支援技術からも実行できるようにします。動きを減らす設定でも操作結果を即座に反映します。
...
入力snapshotはAI生成の完了時刻ではなく、AccountDataが生成contextを確定した時点の件数と最新入力日時を版へ保存します。生成中に追加された入力を次の再生成理由として検出できるようにするためです。本文や入力IDは判定用snapshotへ複製しません。
