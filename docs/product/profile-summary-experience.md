...
見出しを「わたしの傾向」、補足を「これまでの診断と記録から見える、今のあなたです」とします。右上に「表示する版」を置き、既定では最新版を選択します。選択肢には版番号と生成日を表示し、過去版を選んだときは「過去の版を表示中」と明示します。初見で医療的な診断や固定された性格ではないと理解できる短い注意書きを、サマリーカードの前に表示します。
...
版情報には版番号、生成方法、生成日時を表示します。最新版で再生成可能なときは「まとめを更新」を有効にし、診断の追加、日記・記録の追加、前回生成からの時間経過のうち該当する理由を表示します。

生成要求・生成中は、最後に成功した版を残して「新しい版を作成中」と伝えます。失敗時も現在の版を消さず、失敗理由と再試行可能な状態を表示します。過去版では更新操作を無効にし、最新版へ戻るよう案内します。

UI先行PRでは現在のAPI応答を1件の最新版として表示し、版選択と更新操作は無効です。実際の履歴選択、再生成可否、非同期生成状態は後続APIから受け取った時点で有効にします。
...
| 過去版を表示中 | 版番号と生成日を明示し、再生成を無効にして最新版へ戻るよう案内する |
| 再生成可能 | 診断、日記・記録、時間経過のうち該当する理由と「まとめを更新」を表示する |
