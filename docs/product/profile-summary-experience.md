...
通常は最新版の「今のわたし」カードを1枚だけ表示し、履歴があっても背面へカードを常時重ねません。過去版の存在はカード下の控えめな「過去のまとめがあります」ボタンで伝え、押すと1つ前の版を表示します。過去版では「過去のまとめ」と明示し、「今のまとめに戻る」ボタンで最新版へ戻れるようにします。全体の版数や現在位置は強調せず、過去の詳細な探索を画面の主目的にしません。

スワイプは補助操作として残します。最新版を左へスワイプすると1つ前の版へ、過去版を右へスワイプすると1つ新しい版へ移動します。版の移動中だけ切り替え先と同じ内容・寸法のカードを背面に表示し、アニメーション完了後に別の外観へ置き換えません。カードの横位置や高さを切り替えの前後で連続させ、カード下の内容が急に移動するレイアウトシフトを発生させません。

スワイプだけに依存せず、「過去のまとめがあります」と「今のまとめに戻る」を名前付きボタンとして提供し、キーボードと支援技術からも主要な切り替えを行えるようにします。過去版では再生成操作を出しません。
...
| 過去版を表示中 | 「過去のまとめ」と版番号・生成日を明示し、右スワイプまたは「今のまとめに戻る」で最新版へ移動できる |
