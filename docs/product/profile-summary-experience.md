...
複数の完了版がある場合は「今のわたし」カードを重なったカードとして表示します。最新版を手前に置き、カードを左へスワイプすると1つ前の版、右へスワイプすると1つ新しい版へ移動します。表示中のカード内へ版番号、生成方法、生成日をコンパクトに表示し、カード下のページ位置から全体の版数と現在位置を確認できるようにします。

スワイプだけに依存せず、各版を選ぶ名前付きボタンをカード下へ置き、キーボードと支援技術からも同じ切り替えを行えるようにします。過去版では「過去の版」と明示し、再生成操作を出しません。

### 4.3 新しい版の生成

最新版で再生成可能なときは、カードの左奥に「新しい私を見る」カードを一部だけ見せます。最新版を右へ十分にスワイプして離すと生成を要求します。診断の追加、日記・記録の追加、前回生成からの時間経過のうち該当する理由を、生成カード内に短く表示します。

生成要求にはスワイプと同じ名前のボタンも用意し、ポインタ操作が難しい場合にも実行できるようにします。縦スクロール中や短い横移動では生成せず、一定距離を超えて離した場合だけ要求します。
