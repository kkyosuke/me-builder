...
この文書は、本人がこれまでの診断回答と日記から「今の自分にどのような傾向や出来事があるか」を確認する画面の体験を定義します。画面の呼称、表示内容、要約の組み立て方、導線、状態、注意書き、根拠確認と回答修正への遷移を所有します。
...
最初の提供範囲では、完了済み診断の版付き採点結果と、日記会話から非同期生成済みのactiveな`Memory` Brain Itemを使い、次を表示します。

- 複数の診断を横断した短い「今のわたし」
- 診断テーマごとの全パラメータ
- 日記から抽出された最近の出来事
- 要約に使った診断数、診断回答数、日記Memory数、最終記録日時
- 各傾向から個別の回答結果と回答内容へ進む根拠導線
- 回答を増やす、または既存回答を修正する導線
- 本人が確認したセルフケア情報と、AIへ相談する導線

日記Memoryは[Brain Item生成設計](../domain/brain/brain-item-generation-design.md)に従って会話とは別の非同期AI処理で生成します。サマリーの取得時にはAIを呼び出しません。AIによる人物像の自由生成や、異なるパラメータを1つの性格タイプへ統合する処理も行いません。診断は既存の決定的な採点結果から表示用の文章を組み立て、日記は保存済みMemoryをAI由来と分かる形で別枠に表示します。このサマリー自体はBrain Itemとして保存せず、表示時に再構成する読み取りモデルとして扱います。
...
カードの末尾には、サマリーが参照した完了済み診断数、現在有効な診断回答数、activeな日記Memory数、参照した記録のうち最も新しい日時を表示します。これらは統計的な信頼度として見せず、「どの範囲をまとめたか」を示す情報として扱います。
...
activeな日記由来`Memory`を新しい順に最大3件表示します。診断由来の傾向と同じランキングへ混ぜず、「日記からAIが抽出した、本人の発言に含まれる出来事」であることを明示します。各Memoryには記録日時と根拠発言数を表示し、性格、価値観、動機へ言い換えません。
...
次の条件をすべて満たすBrain Itemだけを対象にします。

1. 認証済みAccountが所有する、削除されていないactiveなItemである
2. `category = memory`、`derivation = ai`である
3. `sourceKind = diary`として生成され、削除されていないEvidenceを1件以上持つ

対象は`validFrom`、作成日時、Brain Item IDの順で決定的に並べ、先頭から最大3件を表示します。statementを追加要約せず、保存済みの原文抜粋をそのまま使います。
...
- 日記のAI処理は会話チェックポイントで完了しており、サマリー用の読み取り処理はactiveなMemoryを最大3件選ぶ
- Web UIは返された結果を表示し、独自に採点設定や重みを保持しない
- 診断サマリーの文章とパラメータ結果は保存せず、回答変更時の二重管理を作らない
- 日記Memoryの生成・状態・EvidenceはBrain domainを正とし、プロフィール画面から再生成しない
...
- activeな日記Memoryを診断由来の傾向と区別して表示できる
- サマリー取得時にはAIを呼び出さず、日記Memory生成の失敗や処理中を診断サマリーの失敗にしない
- 画面上に、回答範囲から見える傾向であり医療的な診断ではないことを表示する
- 本人以外の回答、別Accountのキャッシュ、クライアント指定のAccount IDを利用しない
- サマリーの共有・公開、AIによる人物像の自由記述生成を実装範囲へ混ぜない
