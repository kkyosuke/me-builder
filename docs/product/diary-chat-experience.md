...
Phase 1の日記はユーザー起点で始まり、声かけ機能の段階1から固定文面の能動的な入口を追加します。停止の意思は直ちに反映しますが、通常利用では声かけを有効にする操作や送信曜日を選ぶ設定を設けません。配信方式は[日記チャット実装設計 §3.3](../architecture/diary-chat-implementation-design.md#33-18時の基本声かけjob)を正とします。
...
現在の実装対象は段階1です。固定文面、未回答時の休止、本人の停止意思を扱い、曜日別文面や本人情報による個別化を同時に入れません。停止意思は「声かけを止めて」「もう送らないで」のように意味が確定する表現だけを決定的なルールで反映し、曖昧な発言から推定しません。
...
- 停止後に声かけを再開する具体的な操作、時刻候補、個人調整の具体的な選択方法
