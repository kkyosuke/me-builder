...
Phase 1の日記はユーザー起点であり、この能動的な声かけは後続段階の目標体験です。停止の意思は直ちに反映しますが、通常利用では声かけを有効にする操作や送信曜日を選ぶ設定を設けません。配信方式は後続設計で決めます。
...
初回にプロフィール質問を並べず、数回の会話を通じて次の順に把握します。

1. 今の立場と、平日・休日の基本リズム
2. 塾、勤務、部活など曜日に結びつく定期予定
3. 一息つきやすい時間と、答えやすい聞かれ方
4. 趣味、直近の予定、誕生日など、なくても通常の声かけが成立する情報
...
```text
お休みは土日が多い？ それとも週によって変わる？
```

```text
塾や習い事みたいに、曜日が決まっている予定はある？
```

質問への回答がなくても、同じ属性を続けて聞きません。日記の出来事から「毎週月曜は塾」と推定できそうな場合も、声かけへ使う前に「月曜はいつも塾なのかな？」と事実を確認します。後から予定や立場が変わった場合は新しい発言を優先し、古い情報を恒久的なプロフィールとして残しません。

収集の進み具合をユーザーへ達成率として見せたり、情報が少ない人へ質問を増やしたりしません。情報が少ない間は曜日ごとの一般文面を使い、自然な会話を続けられることを優先します。
...
| 4. 個人ごとの調整 | 本人の返信実績による時刻帯・声かけ方針の選択と評価 | 送信頻度の増加、Accountをまたぐ会話本文の学習 |
...
- 声かけコンテキストを一括質問せず、会話の中で優先度順に1件ずつ確認できる
- 情報がない項目を未達として扱わず、一般文面へ戻せる
- 本人ごとの声かけ実績だけを用いて時刻と声かけ方針を選べる
...
- 声かけ停止の具体的な操作、配信基盤、時刻候補、個人調整の具体的な選択方法
