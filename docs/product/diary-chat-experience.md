...
本人が明言したGoalをAIが自動で継続対象にせず、「わたし」の振り返りから本人がGoalと次の小さな一歩を選んだときだけフォローアップを開始します。AIが推定したGoalは選択候補へ出しません。

継続中の項目では、Goalと本人が選んだ次の一歩を表示し、本人が次の一歩を訂正、完了、停止できます。完了・停止した項目をAIの判断だけで再開しません。Liteは同時に1件、Fullは複数を継続でき、現在の話題と関係する1件だけを会話へ使います。利用上限と参照範囲は[サブスクリプション・料金プラン設計](subscription-plan-design.md)を正とします。
