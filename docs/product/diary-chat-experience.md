...
- 日記チャットが担う役割と会話原則
- 日々の声かけの配信判断、文脈選択、段階導入と、振り返りまでの会話フロー
- 出来事から行動原理とその背景を探る方法
...
日記チャットは、本人が許可した日に1日1回だけ声をかけられる体験を目標とします。初期の既定時刻は日本時間の18時とし、本人の予定や返信しやすさに合わせる機能を段階的に追加します。
...
Phase 1の日記はユーザー起点であり、この能動的な声かけは後続段階の目標体験です。本人が声かけを有効にするまでは送信せず、停止は直ちに反映します。通知設定の具体的なUIと配信方式は後続設計で決めます。

### 送るかどうかの判断

18時になったことだけを理由に送信しません。本人の許可、当日の予定、未回答による間隔、進行中の会話を先に確認し、送ってよい場合だけ文面を選びます。

```mermaid
flowchart TD
    A[その日の配信判定時刻] --> B{声かけが有効か}
    B -->|いいえ| Z[送らない]
    B -->|はい| C{選択した曜日か}
    C -->|いいえ| Z
    C -->|はい| D{未回答後の休止中か}
    D -->|はい| Z
    D -->|いいえ| E{進行中の会話や未回答の問いがあるか}
    E -->|はい| Z
    E -->|いいえ| F[利用できる文脈を選ぶ]
    F --> G[声かけを1回送る]
```
...
- 返信のない声かけが3回続いたら能動的な声かけを自動休止し、本人が再開するまで送らない
- 本人からLINEが届いても、会話を始めることと声かけの再開同意を同一視しない
...
声かけの文面は、次の優先順で利用可能な文脈を選びます。使える文脈がない、または使ってよいか確信できない場合は、無理に個別化せず通常の声かけへ戻します。

1. 当日の日中に本人が話し、会話を閉じた後も続きが自然に生じる話題
2. 前日に本人が「明日やる」「また話したい」など継続する意思を示した話題
3. 文脈を使わない、その日の一般的な振り返り
...
曜日や学校、仕事、塾などの定期予定は、まず本人が声かけ設定で指定します。予定そのものを日記本文から自動推定して配信へ使いません。

- 本人は曜日ごとに、送る、送らない、時刻を変える、のいずれかを選べる
- 塾日など名称を付けた予定では、終了後の時刻へずらすか、その日は送らないかを本人が選べる
- 個別予定は、通常の会話で参照できる記憶とは分け、声かけ制御のためだけに使う
- 一時的に休止でき、再開日を指定しない休止も成立させる
- 日中の会話が続いている場合は、予定上の時刻より会話の重複回避を優先する

### 返信しやすい時刻と聞き方を本人ごとに調整する

本人が指定した設定を最優先とし、その範囲内でのみ返信実績を使って調整します。ほかの利用者の会話本文や実績を、その本人の最適化へ流用しません。
...
最適化は返信数だけを増やすことを目的にしません。声かけから会話が始まった割合に加え、停止、連続未回答、すぐ終了した割合、文脈を持ち出されて不快だったという評価も見ます。探索のために送信回数を増やしたり、本人が避けた曜日へ送ったり、強い感情を誘う文面へ変えたりしません。
...
| --- | --- | --- |
| 0. 現在 | ユーザー起点の日記チャット | 能動的な声かけ |
| 1. 18時の基本版 | 明示的な有効化、日本時間18時、1日1回、未回答後の間隔、自動休止、手動停止 | 文脈利用、曜日別時刻、学習による調整 |
| 2. 予定の反映 | 曜日、塾日など本人が指定した予定と一時休止 | 日記本文からの予定推定 |
| 3. 文脈を汲む声かけ | 許可された前日・日中の文脈と通常文面へのフォールバック | 未回答の問いや拒否した話題の持ち越し |
| 4. 個人ごとの調整 | 本人の設定内での時刻帯・声かけ方針の選択と、本人ごとの評価 | 送信頻度の増加、Accountをまたぐ会話本文の学習 |
...
- 声かけが無効な日、対象外の曜日、未回答後の休止中、会話中には送信しない
- 1日に複数の声かけを送らず、連続未回答時に間隔と自動休止を適用できる
- 前日・日中の文脈を使う場合に、拒否・未回答・機微な内容を持ち越さない
- 曜日と塾日など本人が指定した予定を、文脈利用や学習結果より優先できる
- 本人ごとの声かけ実績だけを用い、設定した範囲内で時刻と声かけ方針を選べる
...
- 声かけを受け取ったユーザーの会話開始率
- 時刻帯、曜日、本人指定の予定、声かけ方針、文脈利用有無ごとの会話開始率
...
- 声かけ設定の具体的な画面、配信基盤、時刻候補、個人調整の具体的な選択方法
