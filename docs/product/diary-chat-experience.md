...
現在の実装対象は段階2です。段階1の未回答時の休止、本人の停止意思、新着メッセージによる自動再開を維持したまま、曜日別の一般文面を追加します。本人情報や前日・日中の文脈による個別化は同時に入れません。停止意思は「声かけを止めて」「もう送らないで」のように意味が確定する表現だけを決定的なルールで反映し、曖昧な発言から推定しません。再開専用の表現は要求せず、停止発言より後の新しい日記メッセージを再開の契機とします。
