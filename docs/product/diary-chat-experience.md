...
送信時刻帯の初期候補は日本時間の18時、20時、21時に限定します。本人が「帰宅後」「夕食後」「20時ごろ」のように一息つきやすい時間を明言した場合だけ、まず候補内の対応時刻へ変更します。時刻が不定、候補範囲外、または有効な明言がない場合は18時へ戻します。候補時刻ごとに送信処理を起動しても、Accountごとの送信上限は1日1回のままです。
