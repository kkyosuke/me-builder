...
時刻帯の実績は方針と同じ配送・返信の対応だけを使い、本人ごとの直近90回を上限に集計します。文面と時刻の変更を同時に学習しないため、標準方針3回とほかの各方針2回の初期観測が終わるまでは18時を選びます。その後、18時を3回観測してから20時と21時を各2回ずつ観測します。全時刻帯の初期観測後は、80%を本人内で評価が最も高い時刻、20%を候補全体からの探索に使います。評価値は方針と同じ`(通常返信数 - 2 × 停止数) / 配送機会数`とします。
