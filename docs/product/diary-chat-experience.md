...
現在の実装対象は段階1です。固定文面、未回答時の休止、本人の停止意思と新着メッセージによる自動再開を扱い、曜日別文面や本人情報による個別化を同時に入れません。停止意思は「声かけを止めて」「もう送らないで」のように意味が確定する表現だけを決定的なルールで反映し、曖昧な発言から推定しません。再開専用の表現は要求せず、停止発言より後の新しい日記メッセージを再開の契機とします。
