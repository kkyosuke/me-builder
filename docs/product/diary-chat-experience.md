...
- 助言を選択と振り返りへつなげる「迷いごと比較ノート」の検討案
- 日記チャット体験の完了条件と評価指標
...
Phase 1の日記はユーザー起点で始まり、声かけ機能の段階1から固定文面の能動的な入口を追加します。停止の意思は直ちに反映し、その停止発言より後に本人から新しい日記メッセージが届いた場合は、明示的な再開操作を求めず次の送信可能な日から再開します。停止や再開を受け付けた日に、すでに送信済みまたは停止済みの声かけを追加送信しません。通常利用では声かけを有効にする操作や送信曜日を選ぶ設定を設けません。配信方式は[日記チャット実装設計 §3.3](../architecture/diary-chat-implementation-design.md#33-日次の基本声かけjob)を正とします。
...
本人の返信実績を使い、基本の18時から時刻を調整します。ほかの利用者の会話本文や実績を、その本人の最適化へ流用しません。
...
段階導入では、最初に送信時刻を18時へ固定したまま、本人が明言した聞かれ方だけを声かけ方針へ反映します。次に方針別の返信実績を本人の中だけで蓄積・評価し、その後に送信時刻帯の明言反映と実績による調整を追加します。これにより、文面と時刻の変更を同時に始めず、どの選択が結果へ影響したかを追跡できる状態を保ちます。
...
声かけ方針の実績では、LINEへ配送済みになった声かけだけを返信機会として数え、その後の最初の本人発言だけを直前の未回答配送への返信として対応づけます。送信前の本人発言、skip、failed、同じ発言を複数の過去配送へ重複計上することは禁止します。集計は本人ごとの直近90回を上限とし、通常返信と停止を区別します。

本人が聞かれ方を明言していない場合は、まず標準方針を3回観測し、その後にほかの各方針を2回ずつ観測します。全方針の初期観測後は、80%を本人内で評価が最も高い方針、20%をレビュー済み候補全体からの探索に使います。評価値は`(通常返信数 - 2 × 停止数) / 配送機会数`とし、未回答は分母にだけ含めます。停止・未回答による配信制限を先に適用するため、探索目的で送信回数を増やしません。

送信時刻帯の初期候補は日本時間の18時、20時、21時に限定します。本人が「帰宅後」「夕食後」「20時ごろ」のように一息つきやすい時間を明言した場合だけ、候補内の対応時刻へ変更します。明言した時刻が不定または候補範囲外の場合は18時へ戻し、有効な明言がない場合は本人内の返信実績から選びます。候補時刻ごとに送信処理を起動しても、Accountごとの送信上限は1日1回のままです。

時刻帯の実績は方針と同じ配送・返信の対応だけを使い、本人ごとの直近90回を上限に集計します。文面と時刻の変更を同時に学習しないため、聞かれ方の明言がない場合は、標準方針3回とほかの各方針2回の初期観測が終わるまでは18時を選びます。聞かれ方を本人が明言して方針を固定している場合は、その固定方針を3回観測した後に時刻帯の学習へ進み、利用しない他方針の観測完了は待ちません。その後、18時を3回観測してから20時と21時を各2回ずつ観測します。全時刻帯の初期観測後は、80%を本人内で評価が最も高い時刻、20%を候補全体からの探索に使います。評価値は方針と同じ`(通常返信数 - 2 × 停止数) / 配送機会数`とします。

その日の時刻帯は、最初に処理した候補時刻で1回だけ選び、日本日付ごとの計画として固定します。後続の候補時刻では乱数選択や明言の再評価を行っても計画を差し替えず、同じ選択時刻と選択元を再利用します。本人の明言や返信実績が後から変わった場合は、次の日本日付の計画から反映します。
...
本人が明言したGoalをAIが自動で継続対象にはしません。AIチャットは本人の発言から確認できたGoalと次の小さな一歩を提案でき、本人が明示的に同意したときだけフォローアップを開始します。AIが推定したGoalは提案・選択候補へ出しません。開始後の一覧と操作はWebの「わたし」で管理します。

継続中の項目では、Goalと本人が選んだ次の一歩を表示し、本人が次の一歩を訂正、完了、停止できます。停止した項目は本人の明示操作で再開できます。完了した項目は再開せず、同じ内容へ改めて取り組む場合も新しいGoalとして合意します。完了・停止した項目をAIの判断だけで再開しません。Liteは同時に1件、Fullは複数を継続でき、現在の話題と関係する1件だけを会話へ使います。利用上限と参照範囲は[サブスクリプション・料金プラン設計](subscription-plan-design.md)を正とします。

停止・完了した履歴は本人が元の入力を削除するかAccountを退会するまでWebで閲覧できます。停止・完了と同時にAI Contextから除外し、履歴が残っていることを再利用の根拠にはしません。

実行できなかったこと、完了したこと、停止したことを、本人の性格、意欲、能力の評価へ使いません。Freeへ変更した後も過去の状態は本人が確認できますが、新しい合意、訂正、再開は行いません。
...
現在の会話へ過去情報を加える場合は、本人自身について確認済みの情報と、本人が特定の相手について明言した観察のうち、次の情報だけを候補にします。

- 同じRelationship Categoryで本人が大切にしていたこと
- 本人が確認したRelationship Style、境界、支援の求め方
- 同じ相手または関係について、本人が現在の会話で明示的に参照した過去の出来事
- 同じ相手について本人が以前明言した、休みや生活リズム、好み、対立時の反応などの観察
- 現在も有効なGoal、約束、次に試すと本人が選んだこと

相性関係が成立している場合は、そこで双方が確認できる相手の表示名とRelationship Categoryだけを、現在の発言に明示された「パートナー」などの対象人物を照合する手がかりにできます。照合後も検索対象は現在Accountが所有するBrain Itemに限ります。相性関係の成立や同じファミリーパックへの参加は、相手側の診断結果、日記、Brain Item、AI相談を日記チャットへ利用する根拠にはなりません。相性体験で双方が共有したprojectionも、本人がその内容について相談すると明示した場合を除いて自動では持ち込みません。

会話からは、本人が経験した出来事、本人の感情、選択、Relationship Styleに加え、本人が特定の相手について明言した観察を本人側のMemoryとして保存できます。「パートナーは喧嘩すると一度黙って考える」のような発言は、相手の客観的な性格ではなく本人の観察として残します。AIだけで第三者の性格、病気、意図、行動原理を推定・補完したBrain Itemは生成しません。
...
flowchart LR
    ...
    M --> U[生成時から利用可能]
    B --> U
    U -->|明言抽出Itemを本人が修正| R[修正を新しい根拠として反映]
    U -->|明言抽出Itemを本人が一意に否定| X[無効化して以後使わない]
    U -->|AIの観察と本人の見方が異なる| P[別Itemとして関連づけて併存]
```
...
観察された傾向と本人の自己認識または自己願望が異なる場合は、どちらかを正解として選ばず、前者を観察結果、後者を本人が望む方向として区別します。現在の会話と同じContextのItemがある場合は全体傾向より優先し、Contextが不明な場合だけ全体傾向を使います。反対傾向が複数ある場合も、一方だけを本人らしさとして断定しません。
...
既存のAI相談で整理した選択肢を、本人が後から見返せる1枚のノートへ保存する案です。「誘いを受けるか断るか」のような日常の迷いを、自分の価値観や過去の経験と照らして考え、決めた理由とその後の感想まで残せることを目指します。提供時期と利用できるPlanは未決定です。

最小構成の候補は次のとおりです。

- 比較したい選択肢2つと、今回大切にしたいこと
- 各選択肢について大切にしたいことに合う点、気になる点、まだ分からない点
- 比較に使った本人の過去の記録と、その時点が分かる参照
- 本人が選んだ結論と理由、後から本人が追記する感想

本人がAI相談の整理内容を確認して保存し、Webで見返せる導線を候補にします。結論を保留したまま残すことも認め、AIが選択やその後の結果を本人に代わって確定しない体験にします。助言の根拠と不確実性は本節の規則に従います。

```mermaid
flowchart LR
    C[AI相談で迷いを整理] --> N[比較ノートを確認して保存]
    N --> D[本人が選択と理由を残す]
    D --> R[後から感想を追記]
    D -->|本人が希望する場合| G[既存の行動フォローアップ]
```

選んだ行動の継続支援には既存の行動フォローアップとの接続を検討します。利用権限は[サブスクリプション・料金プラン設計](subscription-plan-design.md#41-機能一覧)を正とし、ノート保存とフォロー開始は別の操作にします。会話からGoalを含むBrain Itemを生成する際の規則は[Brain Item生成設計](../domain/brain/brain-item-generation-design.md)に従います。新たなドメイン分類は前提にせず、[Brain内部情報の分類](../domain/brain/brain-content-taxonomy.md)との対応は後続設計で整理します。

保存したノートでは、本人の結論・理由・感想と、AIが整理した比較内容を区別する候補にします。根拠となる記録の訂正・削除後も古い内容を再利用する経路にしないよう、[Source Recordのライフサイクル設計](../domain/source/source-record-lifecycle-design.md)との整合を、保存・再表示・更新の設計で確認します。

検証では、比較後に「選ぶ理由が整理できた」と感じた割合、保存したノートを見返した割合、その後の感想を残した割合を確認します。ノート自体の保存・訂正・削除と根拠変更の波及、会話から保存する操作、行動フォローアップへの接続方法は着手時に具体化します。
...
1回のAI入力へ追加する過去情報は、Brain Itemを最大5件、Source Recordの原文を最大3件とします。件数内へ収まらない場合は、現在の状況との関連性、Contextの一致、現在も有効か、EvidenceとConfidenceを優先して選びます。低ConfidenceのItemは一律に除外せず順位を下げ、必要な場合も弱い仮説として区別します。
...
- 相手の気持ち、性格、病気、意図を断定しない。本人が過去に明言した観察を使う場合も「以前そう話していた」という本人視点と不確実性を保つ
...
- 本人についての`highly_sensitive`なAI推定は、本人が現在同じ話題を扱っている場合だけContext Packageへ含め、別の話題から持ち出さない
- `highly_sensitive`なAI推定を、日次の声かけ、Push通知、相性共有、ファミリーパック内共有、外部MCPへ利用しない
- 医療、法律、金融、自傷他害など高い安全性が必要な相談では、日記チャットの推定だけで断定的な助言をしない
- 医療的な診断、治療方針、服薬の開始・中止・変更を指示せず、日記チャットを医療や専門支援の代替として扱わない
...
高リスクな相談でも本人の発言は通常の日記・会話と同じ原本として保存し、通常の訂正・削除規則に従います。安全経路の判定と応答結果は本文を運用ログへ複製せずに原本と対応づけ、特別な保存期間や有人通知は設けません。

初期提供地域は日本です。差し迫った危険がある場合だけ119・110と身近な信頼できる人への連絡を優先して案内し、それ以外の高リスク相談では厚生労働省など公的機関が公開する国内の相談先を示します。電話番号や相談先をAIに生成させず、運営が確認した情報だけを表示します。安全経路へ切り替えても運営者や第三者へ自動連絡しません。
...
AI推定だけを根拠に「本人らしい」と断定した助言を返しません。AI推定を使う場合は、推定であることと根拠を示します。本人が異なる自己認識や自己願望を述べた場合は元の観察Itemを上書きせず、別の見方として併存させます。専用の本人確認画面は前提にしません。
...
現在は段階4まで実装済みです。本人が会話で明言した高優先属性、曜日・当日・前日の文脈に加え、本人の明言または本人内の返信実績から時刻帯と声かけ方針を自動選択します。段階1の未回答時の休止、本人の停止意思、新着メッセージによる自動再開は維持します。停止意思は「声かけを止めて」「もう送らないで」のように意味が確定する表現だけを決定的なルールで反映し、曖昧な発言から推定しません。再開専用の表現は要求せず、停止発言より後の新しい日記メッセージを再開の契機とします。
...
- 会話から第三者の人物像を推定せず、本人の経験とRelationship Style、および本人が明言した特定の相手についての観察だけを記録できる
- 「パートナー」など現在の発言に明示された対象を、相性共有の表示名とRelationship Categoryで照合し、現在Account内のVector検索結果だけを助言へ利用できる
