...
- 各プランへ付与する利用権限
- AI利用上限へ到達した場合の利用者体験
- トライアル、アップグレード、ダウングレード、解約、返金後の扱い
...
最初の有料提供では、次の4プランを用意します。価格は利用者が支払う初期提供の総額であり、消費税や決済手数料を別途加算しません。一般公開前に[§11](#11-価格と継続価値の検証)の実利用結果を確認し、変更する場合はこの文書を先に更新します。
...
初期公開は18歳以上だけを対象とします。生年月日は保存せず、規約同意、有料Planの購入、ファミリーパックの招待承諾で本人が18歳以上であることを確認します。未成年者本人の利用、法定代理人による同意・代理管理、子どもの記録閲覧は初期提供に含めません。
...
- 本人データの確認、訂正、削除、エクスポート
- 相手との共有内容の確認と共有終了
- 通知と日々の声かけの停止
- 解約と、現在の契約状態の確認
- 差し迫った危険がある場合の安全案内

未実装の訂正、削除、エクスポートは、有料化前の提供条件として[§12](#12-有料化前の提供条件)で追跡します。
...
ファミリーパックは、最大4つの有効なAccountへFull相当の利用権限を付与します。支払者と参加者にも[初期公開の年齢範囲](#2-結論)を適用し、代理管理や他の参加者による記録閲覧を認めません。
...
支払者が確認できるのは、参加者の表示名、招待状態、席の利用状態、請求状態だけです。次は確認できません。
...
Lite、Full、ファミリーパックには、全Planを通じてAccountごとに最初の1回だけ14日間の無料トライアルを用意します。Account作成時に自動開始せず、本人が対象プラン、終了日、終了後の価格、自動更新を確認し、支払方法を登録して開始します。

- トライアル中は選んだプランと同じ利用権限を付与する
- ファミリーパックの参加者は、支払者のトライアル期間を共有する
- 終了前に停止した場合も終了日時までは利用できる
- 終了日の7日前までに終了日、終了後の価格、解約方法を登録メールアドレスへ通知する
- 終了後に課金する条件、自動更新、停止期限は決済画面と規約で明示する
- トライアルを繰り返すためのAccount再作成やファミリーパック移動を許可しない

ファミリーパックのトライアル中に席を有効化した参加者も、トライアルを利用したAccountとして扱います。退出、再参加、別パックへの移動、Customerの再作成では利用可能に戻しません。終了後の自動更新へ本人が明示的に同意した場合だけ課金し、支払方法を登録しないトライアルは初期提供では行いません。
...
利用者都合の解約、更新後の残期間、期間末に予約したダウングレードについて、日割り返金は行いません。二重請求、運営者または決済処理の誤り、適用法令上必要な場合、運営者の責任で有料機能を長期間提供できなかった場合は、請求事実と影響期間を確認して全額または一部を返金します。

返金しても本人データを削除しません。返金により契約を終了する場合はFreeへ戻し、返金だけを理由にAccountの利用停止や本人データの削除を行いません。公開ページ、最終確認画面、規約では、返金条件と申出方法を同じ内容で表示します。

### 8.5 ファミリーパックからの退出

支払者が参加者を席から外した場合、対象AccountはFreeへ戻り、本人データを保持します。支払者は参加者のAccountを削除できません。参加者本人はいつでもパックから退出でき、退出した事実は席管理に必要な範囲で支払者へ表示します。ファミリーパックを解約した場合は、契約期間の終了時にすべての参加AccountをFreeへ戻します。

退出、席からの削除、契約終了後も、新しい招待を承諾すれば待機期間なしでファミリーパックへ再参加できます。過去の招待は再利用せず、同時に参加できるパックは1つだけとします。再参加や別パックへの移動でトライアル利用履歴は戻りません。短期間の席入れ替えによる濫用が確認された場合は、利用状況を確認してから待機期間の導入を別途判断します。
