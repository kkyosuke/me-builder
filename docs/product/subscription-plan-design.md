...
Checkout Session作成の冪等性はAccount、Stripe Customer、直前のCheckout Sessionを世代として管理します。sandboxの再作成などでCustomerを置き換えた場合は新しい世代の冪等キーを使い、旧Customer向けリクエストとのパラメータ不一致を起こさないものとします。
