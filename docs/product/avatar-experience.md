...
認証済みAccountの画像を保存・取得・削除するバックエンド契約は[プロフィールAPI契約](../development/profile-api.md)で定義します。現在のWeb UIはまだこのAPIへ接続せず、画面内状態だけを使います。
...
| 外部AIサービス | 利用しない |
| API Server / 共有D1 / Private R2 | 保存契約を提供する。現在のWeb UIからの接続は後続変更で行う |
...
- 永続化時は拡張子だけを信頼せず、実データのheader形式、容量、寸法をサーバー側でも検査する
