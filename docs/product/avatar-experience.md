...
画像選択は現在値の変更ではありません。利用者がプレビューを確認して「この画像を保存」を押したときだけ現在値を更新し、モーダルを閉じます。閉じる操作では選択途中の画像を破棄し、現在値を維持します。
...
1. タイトルと閉じる操作
2. 現在のアバター。画像選択後は同じ表示領域を設定予定の画像へ切り替える
3. 端末から画像を選ぶ操作
4. 「この画像を保存」または「LINEの画像に戻す」

画像選択によってモーダルの高さや主要操作の位置を変えないため、モーダル上部の画像表示領域を現在値から設定予定のプレビューへ切り替えます。選択後は見出しを「設定するアバター」、説明を保存前であることが分かる内容へ変えます。
...
| ファイル選択済み | 現在画像と同じ領域を、中央を正方形に切り抜いた設定予定の画像へ切り替える。選び直しと保存操作を表示し、ファイル名は表示しない |
...
- 選択画像を外部AIサービスへ送信しない
- 選択画像は本人が「この画像を保存」と確定した場合だけプロフィールAPIへ送信する
...
- ファイル選択後もモーダルの高さと主要操作の位置が変わらず、上部の画像と説明だけが設定予定の内容へ切り替わる
