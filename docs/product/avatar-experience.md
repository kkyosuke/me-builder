...
サーバー保存、再読み込み後の復元、端末間同期、画像の切り抜き・回転・拡大縮小は現在のUI提供範囲に含めません。
...
| ファイル選択済み | 円形プレビュー、ファイル名、選び直し、設定操作を表示 |
...
現在はUI確認を目的とし、選択画像をReactの画面内状態だけに保持します。再読み込みするとアプリで選択した画像は失われ、LINEプロフィール画像またはプレースホルダーへ戻ります。

将来のバックエンド接続では、認証済みAccountの画像として安全に保存・取得・削除できるようにします。ただし、API、保存先、画像正規化、容量上限の具体形はこの変更では決めません。
