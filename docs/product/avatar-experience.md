...
- Account上限へ達した場合は次に利用できる時期を案内し、全体上限へ達した場合は一時的に利用できないことを案内する。どちらの場合も画像をそのまま設定する操作は利用できる
...
| Account生成上限 | 次に生成できる時期と、アップロード画像をそのまま設定する操作 |
| 全体利用上限 | 一時的にAI生成を利用できない説明と、アップロード画像をそのまま設定する操作 |
...
- Account単位または環境全体の生成上限へ達した場合、AI生成だけを停止し、画像をそのまま設定できる
