...
同意の保存完了後は、開始元にかかわらず「わたしのまとめ」へ遷移します。同意済みのversionが現在の同意必須version以降である間は、起動のたびに同意画面を出しません。重要な改定では新しい同意必須versionを公開し、同じゲートで再同意を求めます。軽微な改定では公開versionだけを追加し、既存利用者の利用を止めません。プロフィールからは、現在の本文と、本人が同意した全履歴をいつでも確認できます。
