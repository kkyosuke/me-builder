...
この文書は、me-builderの利用者がLINEとWebを行き来し、日記、診断、相性、自分の傾向、プロフィール設定、セルフケア、AI相談へ到達するまでの全体画面遷移を定義します。チャネルをまたぐ入口、Webの主ナビゲーション、共通ヘッダー右上の入口、各画面の接続、認証後の復帰先、共通の例外遷移、および一般利用者と管理者の画面分離を所有します。
...
| アバターのアップロード、プレビュー、設定 | [アバター設定体験設計](avatar-experience.md) |
...
- Webは、診断への回答、個別の回答結果、相性、自分の傾向、プロフィール設定、セルフケア情報、AI相談を担当する
...
flowchart LR
    ...
    P -->|アバターを押す| A[アバター設定モーダル]
    A -->|閉じる・送信・設定完了| P
    P -->|ライト・ダークを選択| P
    P -->|戻る| S
```

共通ヘッダー右上には、現在のアバターまたはプレースホルダーを使うプロフィール操作を置きます。プロフィールでアバターの現在値と表示テーマを確認し、アバターの詳細編集はモーダルで行います。入口、テーマ選択、開始元への復帰は[プロフィール設定体験設計](profile-settings-experience.md)を正とします。
...
「わたしの傾向」は、診断結果の閲覧、次の診断、セルフケア、AI相談を接続する本人向けのハブです。ただし、MCP接続管理、公開範囲、エクスポートなど画面設計が未確定の機能を、この図だけで新しい遷移先として確定しません。
...
| 共通 | プロフィール | 共通ヘッダー右上、直接リンク | 開始元、アバター設定モーダル、同画面内のテーマ選択 |
| 共通 | アバター設定モーダル | プロフィールのアバター | プロフィール、同モーダル内の画像選択・保存 |
| わたし | わたしの傾向 | リッチメニュー、主ナビゲーション、個別の回答結果 | 個別の回答結果、診断一覧、セルフケア、AI相談 |
...
- プロフィールを閉じた場合は開始元へ戻る。アバター設定モーダルは保存成功時または中断時に閉じてもプロフィールを維持する
