...
この文書は、me-builderの利用者がLINEとWebを行き来し、日記、診断、自分の傾向、セルフケア、AI相談へ到達するまでの全体画面遷移を定義します。チャネルをまたぐ入口、Webの主ナビゲーション、各画面の接続、認証後の復帰先、共通の例外遷移、および一般利用者と管理者の画面分離を所有します。
...
- Webは、診断への回答、個別の回答結果、自分の傾向、セルフケア情報、AI相談を担当する
- Webの主ナビゲーションは「診断」と「わたし」の2項目にする
...
Webの一般利用者向け画面では、次の2項目を同じ位置に表示します。
...
- 認証前に要求された診断、わたし画面、管理者画面の区別を失わない
...
- 開始元を復元できない直接リンクでは、領域のルート画面へ戻す。診断は診断一覧、本人向け情報はわたしの傾向とする
