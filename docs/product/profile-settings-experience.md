...
この文書は、本人がWebの共通ヘッダー右上から「プロフィール」を開き、アバターと表示テーマを確認・変更する体験を定義します。プロフィールへの入口、画面構成、設定項目のまとまり、ライト・ダークテーマの選択と保存、開始元へ戻る規則、画面状態、および完了条件を所有します。
...
- 画面タイトルは「プロフィール」とし、「アバター」と「表示」の2セクションを置く
...
プロフィールのアバター行には、現在のアバター、未設定のプレースホルダー、または候補完成の短い状態だけを表示します。画像の選択、切り抜き、AI生成の説明、候補比較、削除はプロフィール画面へ展開せず、[アバター設定体験設計](avatar-experience.md)が所有する専用画面で行います。

候補が完成していて未選択の場合は「候補ができました」と表示し、「変更」から同じジョブの候補へ復帰できるようにします。候補が完成しただけではプロフィール入口や現在値を新しい画像へ変えません。
...
| アバター設定済み | 現在の画像と「変更」 |
| アバター生成中 | 現在値を維持し、「生成中」と専用画面への導線 |
| アバター候補完成 | 現在値を維持し、「候補ができました」と専用画面への導線 |
