...
この文書は、本人がWebの共通ヘッダー右上から「プロフィール」を開き、アバターと表示テーマを確認・変更する体験を定義します。プロフィールへの入口、画面構成、設定項目のまとまり、管理者画面への導線、ライト・ダークテーマの選択と保存、開始元へ戻る規則、画面状態、および完了条件を所有します。

アバター画像のアップロード、AI生成、候補選択、保持、削除は[アバター設定体験設計](avatar-experience.md)、Web全体での画面接続は[全体画面遷移設計](screen-navigation.md)を正とします。

この文書は、画像処理の詳細、配色トークン、公開プロフィール、表示名の編集、Accountの認証情報、APIやデータベースの具体的なスキーマを所有しません。

## 2. 結論

- 本人確認後の一般利用者向けWebでは、共通ヘッダー右上にプロフィール操作を置く
- アバター設定済みなら現在のアバター、未設定なら共通の人物プレースホルダーを入口へ表示する
...
- アバター行には現在値と状態を表示し、アバター自体から画像送信または候補選択のモーダルを開く
- 表示テーマは「ライト」と「ダーク」の2択とし、選択時に画面全体へ即時反映する
- テーマは同じブラウザへ保存する。初期提供ではAccountへ保存せず、端末間で同期しない
- プロフィールを閉じると開始元へ戻り、アバターのAI生成中に閉じても生成処理は続ける
...
- 共通ヘッダー右上のプロフィール操作
- 現在のアバターまたはプレースホルダーの表示
- アバター設定モーダルへの導線と、設定後の現在値更新
- ライトテーマとダークテーマの明示的な選択
- 選択テーマの即時反映と同じブラウザでの復元
- 開始元を保ったプロフィールの表示と終了
- 検証済みのAccount roleが`admin`の場合に限った管理者画面への導線
- 読み込み、保存不能、アバター生成中・候補完成の状態表示

表示名、自己紹介、公開範囲、ログアウト、Account削除は、所有する体験が未設計なので初期提供へ混ぜません。「プロフィール」という名称だけを根拠に空の項目や操作不能な行を並べません。
...
flowchart TD
    ...
    P -->|アバターを押す| A[画像送信・候補選択モーダル]
    A -->|送信・設定完了・閉じる| P
    P -->|ライト・ダークを選択| P
    P -->|管理者だけ| M[管理者画面]
    P -->|戻る| S
    D[プロフィールへの直接リンク] --> P
    P -->|開始元を復元できない| W[わたしの傾向]
```

- プロフィールを開いたときは開始元をブラウザ履歴として保ち、戻る操作で同じ画面へ戻す
- アバターモーダルを閉じる場合はプロフィールを維持し、その後にプロフィールの開始元へ戻れるようにする
- プロフィールを直接開いた場合や開始元を復元できない場合は「わたしの傾向」を安全な戻り先にする
- 戻る操作でテーマ選択を取り消さない。選択した時点で確定済みとして扱う
- アバターのアップロード確認など、破棄される未確定操作がある画面ではアバター設計の確認規則を優先する
- AI生成中にプロフィールまたはアバター設定を閉じてもジョブを中止せず、再訪時に状態を復元する
...
プロフィールのアバター行には、現在のアバター、未設定のプレースホルダー、生成中または候補完成の短い状態だけを表示します。画像の選択、AI生成の説明、候補比較、削除はプロフィール画面へ直接展開せず、[アバター設定体験設計](avatar-experience.md)が所有するモーダルで行います。

生成中はアバターへ処理中表示を重ねて新しい操作を抑止します。候補が完成していて未選択の場合は「候補ができました」と表示し、アバターから同じジョブの候補へ復帰できるようにします。候補が完成しただけではプロフィール入口や現在値を新しい画像へ変えません。
...
同じブラウザでAccountが切り替わってもテーマは維持されます。一方、アバターと生成状態はAccountごとに再取得し、以前のAccountの画像を表示しません。
...
| アバター未設定 | プレースホルダーと「設定する」 |
| アバター設定済み | 現在の画像と「変更」 |
| アバター生成中 | 現在値を維持し、アバター上の処理中表示と「アバターを生成中」。新しい送信は抑止 |
| アバター候補完成 | 現在値を維持し、「候補ができました」と候補選択モーダルへの導線 |
| アバター取得失敗 | プレースホルダーへ安全に退避し、再試行。壊れた画像を表示し続けない |
| テーマ保存済み | 現在のライトまたはダークを選択済みで表示 |
| テーマ保存不能 | 選択をページ内では反映し、次回復元できない可能性だけを短く伝える |
| 通信失敗 | テーマ変更は利用可能なままにし、アバターだけを再取得できるようにする |
| 管理者 | 「管理」と管理者画面へのリンクを表示 |
| 一般利用者・role取得不能 | 管理者向けのセクションとリンクを表示しない |

アバターの通信失敗でプロフィール画面全体を利用不能にしません。テーマはサーバー通信へ依存せず、画像の取得状態とは別に操作できます。
...
flowchart LR
    ...
    B --> V[Web全体の表示]
    P -->|アバター操作| A[アバター設定モーダル]
    P -->|検証済みroleがadmin| M[管理者画面]
    A --> D[Account所有の現在値・生成状態]
    D --> P
    D --> H
```
...
- アバター機能はAccount所有の画像と生成状態を担当し、詳細はアバター設計へ閉じる
- Web UIはテーマをAccount設定とみなしてAPIへ送らず、アバターのAccount IDをクライアント指定しない
...
## 10. アクセシビリティとレスポンシブ表示

- 右上の入口は画像の代替テキストではなく、操作として「プロフィールを開く」と伝える
- プロフィール画面の戻る、アバター変更、テーマ選択をキーボードとタッチの両方で操作できる
...
- プロフィールのアバターから設定モーダルを開き、送信、設定または中断後もプロフィールへ戻れる
- AI生成中に画面を離れても処理が続き、再訪時に同じ状態へ復帰できる
- ライトまたはダークを明示的に選び、Web全体へ即時反映できる
- 同じブラウザで次回開いたとき、保存したテーマが初回描画前から復元される
- テーマをAccountへ保存せず、アバターは別Accountへ切り替わったときに混ざらない
- プロフィールを閉じると開始元へ戻り、直接リンクでは「わたしの傾向」へ退避できる
...
- ログアウト、Account削除、データエクスポート、MCP接続管理
