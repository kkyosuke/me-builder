...
有料Planは金色の共通表現でプレミアムな状態を示しつつ、LiteとFullの文字を残して現在Planを正確に判別できるようにします。ファミリーパックは色だけでなく複数人アイコンを併記します。Planが読み込み中または取得失敗の場合はFreeと推測せず、バッジを表示しません。
...
| Plan取得済み | 右上の入口へ現在Planの文字付きバッジを表示 |
| Plan読み込み中・取得失敗 | 誤ったPlanを示さないようバッジを表示せず、プロフィール入口は利用可能なままにする |
