...
招待画面のカテゴリは`category`、本人向け共有内容確認のカテゴリは`shareCategory` query parameterへ反映します。再読み込みとブラウザの履歴移動ではURLから選択状態を復元し、未指定または定義外の値は「パートナー」として扱います。招待画面から「わたし」へ移動するリンクは、選択中のカテゴリを`shareCategory`へ引き継ぎます。
