...
画面上で本人を表すアイコンには、プロフィール設定で選んだ画像を使います。設定画像がなければLINEプロフィール画像へ戻し、LINEにも画像がなければLINEの既定表示に合わせた人物シルエットを表示します。招待の確認画面では送信者と受信者の双方へ同じ優先順位を適用します。
...
送信者のアイコンは、招待が参照する送信者Accountをサーバー側で確定してから取得します。受信者が送信者のLINE認証情報を持つことや、クライアントが相手のAccount IDを指定することはありません。アイコンを取得できない場合も招待内容の確認は継続し、人物シルエットへ縮退します。
...
共有用の文章は具体的な出来事、日時、場所、人物・組織名、日記や会話の引用、健康状態を含めず、記録から一般化できる振る舞い・考え方だけにします。生成結果は自動共有せず、本人が実際の全文を確認して招待発行または再同意した場合だけ共有対象にします。
...
初期提供では、本人による文章の自由編集と自由記述の追加を行いません。本人は、表示された実際の文章と共有されない詳細を確認し、利用できる共有専用projectionと診断由来の振る舞い・考え方をすべて共有します。
...
同意は共有専用プロフィール版、確認画面に表示した文章、テーマ名、軸、本人の位置を含む表示内容に対して行います。新しいプロフィール版の生成、回答修正、採点設定の更新で表示内容が変わっても自動共有せず、プロフィール所有者本人の再確認を求めます。共有文が依存する回答、Brain Item、日記、Source Recordが削除または無効化された場合は古い文章を直ちに非表示にします。再確認が終わるまで、本人には更新操作を、相手には相手側の準備待ちであることだけを表示します。
