...
リンク発行前は、固定フッターに「共有して招待リンクを発行する」を表示します。発行後は同じ固定フッターの操作領域を「友だちに送る」と「コピー」の横並びへ切り替え、フッターの高さを変えません。「友だちに送る」はLINEの共有先選択を開き、利用できない環境では端末の共有先選択を開きます。共有先選択を閉じた場合は送信済みと扱わず、キャンセルしたことを画面内に表示します。「コピー」は招待URLをクリップボードへコピーします。送信文は、誰からの招待か、相性診断へのリンクであること、承諾するまで情報共有が始まらないことが分かる短い定型文にします。
