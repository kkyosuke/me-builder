...
相性から日常の実践へつなげる「ふたりの『試してみること』」の検討案も、この文書で扱います。
...
#### 5.4.5 ふたりの「試してみること」（検討候補）

既存の「2人で話すテーマ」から、双方が試したい小さな行動を選び、後から振り返る案です。相性の違いを知った後、生活の中でどう関わるかを2人で決められることを目指します。提供時期と利用できるPlanは未決定です。

たとえば、予定の決め方の違いについて話し、「週末の予定は木曜に相談する」を双方で選びます。その後に「続けたい」「変えたい」「やめたい」をそれぞれ表せる体験を候補にします。この行動例は、診断結果から導く正解や必須の約束ではありません。

最小構成の候補は次のとおりです。

- 現在共有できる相性シートの話すテーマを入口にする
- 審査済みの短い行動候補から、双方が確認した1件を保存する
- 試した後の感想を選択式で残し、次にどうするかを話せるようにする
- 片方が中止を選べば続行を強制せず、未回答も合意や失敗として扱わない

追加で共有する情報の種類は、行動候補の選択と本人が入力した選択式の振り返りに限定する案です。共有開始前の双方の同意は、[§6](#6-同意と共有終了)の「共有する情報の種類を実質的に広げる場合」の規則に従います。行動を一緒に選ぶ操作と、情報共有への同意は区別します。

話すテーマの入力として利用できる情報は[前節](#544-2人の継続的な振り返り)に従い、既存の個人向け行動フォローアップを相手へ公開しません。自由記述の送り合い、AIによる合意内容の生成、達成率での関係評価は最小構成に含めません。実践の中止と相性共有の終了を区別し、共有終了後の閲覧可否は[§6](#6-同意と共有終了)に従う候補とします。

検証では、話すテーマの閲覧から双方による行動選択へ進んだ割合、実践後に振り返った割合、2人の会話に役立ったという評価を確認します。未回答の相手を急かす通知は前提にしません。

着手時には、追加共有の確認方法、行動候補の編集・審査、片方の中止と共有終了時の表示・削除、Plan変更時の扱いを具体化します。既存の相性シートに対する同意・保存範囲を、この検討案だけで変更しません。
...
招待リンクの有効期間は発行から14日です。招待は宛先Accountを事前指定しないため、リンクを受け取って最初に承諾した認証済みAccountを共有相手とします。転送されたリンクでも同じであり、2人目以降は承諾できません。発行画面では、送りたい相手以外へ転送しないことと、最初の承諾者が相手になることをリンク発行前から明示します。初期提供では相手のブロック・通報機能を設けず、送信者の招待取消と双方の共有終了を停止手段とします。
...
- 転送された招待も最初の承諾者との関係として扱い、その後は本来の受信予定者を含む他Accountへ招待内容や共有内容を返さない
