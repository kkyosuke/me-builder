...
Questionnaire domainは次の3集約で構成します。

| 集約 | 担当すること |
| --- | --- |
| `Question` | 質問の同一性、内容の版、選択肢、審査状態を守る |
| `Survey` | 回答してもらう質問の組み合わせ、順序、受付期間、公開状態を守る |
| `SurveyResponse` | 1つのAccountによる1つのSurveyへの現在の回答と進捗を守る |

1問の回答はQuestionnaire domain上の`Answer`であると同時に、Source domain上では1件の`Source Record`です。`Answer`を独立した集約にはせず、`SurveyResponse`が現在有効な回答と対応するSource Recordを管理します。

「あとで回答」は回答内容ではありません。本人についての新しい命題を持たないためSource Recordを作らず、SurveyResponseの進捗として記録します。

## 3. ドメインの責務と境界

### Questionnaire domainが担当すること

- 運営が質問を作成、審査、改訂する
- 公開済みの質問内容と選択肢を不変にする
- 質問の特定の版を並べてSurveyを構成する
- Surveyの公開と受付期間を管理する
- 本人が選んだ選択肢を質問の版に対して検証する
- Accountごとの回答進捗と現在有効な回答を管理する
- 回答の新規作成、修正、削除をSource domainへ依頼する

### Questionnaire domainが担当しないこと
...
Questionnaire domainは「誰が回答したか」をクライアントから受け取りません。操作主体は、サーバーが検証した本人性からAccount domainを通じて解決します。

## 4. 全体像

```mermaid
flowchart LR
    subgraph QD[Questionnaire domain]
        Q[Question]
        QV[Question Version]
        C[Choices]
        S[Survey]
        SQ[Ordered Survey Questions]
        R[SurveyResponse]
        A[Current Answers]
        Q --> QV
        QV --> C
    ...
    AC[Account] -->|owns one response per survey| R
    A -->|one answer = one record| SR[Source Record]
    BI[Brain Item] -->|Phase 2: evidence| SR
```

SurveyはQuestionそのものではなく、必ず特定のQuestion Versionを参照します。これにより質問が改訂されても、公開済みSurveyと既存回答の意味が変わりません。
...
Phase 1の回答形式は`single_choice`だけとし、スワイプアンケートでは選択肢を2件に限定します。各選択肢は版の中で一意な`Choice ID`、表示文言、任意の表示用メタデータを持ちます。
...
stateDiagram-v2
    ...
    approved --> retired: 新規Surveyでの利用を終了
```

- `draft`は編集できる
- `approved`へ移した内容は不変とする
- `approved`だけをSurveyの公開内容として確定できる
- 訂正は既存版の変更ではなく新しい版の追加で行う
- `retired`は新しいSurveyへ追加できないが、既存Surveyと回答から参照できる
...
`Survey`は、ユーザーへ1回の回答単位として提示する質問セットです。次を持つ論理モデルです。

- タイトル
- 回答受付の開始時点と終了時点
- 公開状態
- 順序づけられた`Survey Question`

Survey QuestionはSurvey内の項目であり、特定のQuestion Versionへの参照と表示順を持ちます。将来同じQuestion Versionを別のSurveyで再利用しても、回答がどのSurvey上の項目に対するものかを区別できます。
...
`published`のSurveyについて、現在時刻と受付期間から次の可用状態を導出します。
...
受付終了や公開停止の後も、既に回答した本人は回答内容を確認できます。公開停止は誤ったSurveyを緊急停止するための状態であり、内容を変更して再公開する操作には使いません。

### 不変条件

- Surveyは1件以上のSurvey Questionを持つ
- 同じQuestionを1つのSurveyへ重複して含めない
- Survey公開時点で、すべてのSurvey QuestionがapprovedのQuestion Versionを参照している
- 受付終了時点がある場合、受付開始時点より後にする
- published以降はタイトル、受付期間、質問の組み合わせ、参照する版、順序を変更しない
- 公開内容の訂正が必要な場合は新しいSurveyを作成する

Phase 1では公開中のSurveyをすべての有効なAccountが回答できます。対象者のセグメント、招待制、質問のランダム化は後続設計へ延期します。

## 7. `SurveyResponse` aggregate

### 構成と同一性

`SurveyResponse`は、1つのAccountによる1つのSurveyへの回答全体を表す集約ルートです。同じAccountとSurveyの組み合わせに複数のSurveyResponseを作りません。回答のやり直しは別Responseの追加ではなく、現在の回答の修正として扱います。

SurveyResponseは次を管理します。

- 対象のAccountとSurvey
- Survey Questionごとの現在有効なAnswer
- 「あとで回答」を選んだSurvey Questionとその操作時点
- 回答進捗

SurveyResponseは最初の回答または「あとで回答」の操作時に作成します。一覧を表示しただけでは作成しません。

### 回答状態

回答状態は現在有効なAnswerの件数から導出し、独立した真実として重複管理しません。Phase 1ではSurvey内の全問を回答対象とします。

| 状態 | 条件 |
| --- | --- |
| 未回答 | 現在有効なAnswerが0件 |
| 回答途中 | 1件以上あるが全問分はない |
| 回答済み | 全Survey Questionに現在有効なAnswerがある |
...
- どのSurvey Questionに答えたか
- そのSurvey Questionが参照するどのQuestion Versionか
- どのChoice IDを選んだか
- サーバーが受け付けた時点
- 対応するSource Record

クライアント時刻は表示や診断の参考にはできますが、回答時点の正にはしません。Question VersionとChoice IDがSurveyに含まれる組み合わせかをサーバー側で検証します。

### 操作

#### 新規回答

1. Accountが有効でSurveyが受付中であることを確認する
2. Survey Question、Question Version、Choice IDの組み合わせを検証する
3. 本人入力のSource Recordを1件作る
4. SurveyResponseの現在有効なAnswerとして対応づける
...
「あとで回答」はSurveyResponseへ延期の事実だけを記録し、AnswerおよびSource Recordを作りません。後から回答した時点で延期状態を解消します。

### 不変条件

- AccountとSurveyの組み合わせにSurveyResponseは最大1件
- Survey Questionごとに現在有効なAnswerは最大1件
...
- 回答時点のSurvey Question
- 回答時点のQuestion Version
- 選択したChoice ID
- サーバーが回答を受け付けた時点

質問文や選択肢の表示文言は不変なQuestion Versionから復元できます。Source Recordへ同じ文言を独立した正として複製しません。ただしエクスポートや長期保持でQuestionnaire domainから独立したスナップショットが必要かは、物理データ設計時に判断します。
...
- 回答できるのは有効なAccountだけ
- SurveyResponseと回答Source Recordの所有者は同じAccount
- LIFFまたはLINE Loginの検証結果からAccountを解決する
- 一覧、回答、修正、削除のすべてで同じ本人性の規則を使う
- Accountが見つからない場合はSurveyResponseやSource Recordを作らない

現在のLIFFセッション解決はAccountの存在確認までを行います。継続するAPI呼び出しで本人性を安全に再利用するセッション方式は、アンケートAPI実装前に決める必要があります。
...
### SurveyがQuestionだけを参照し、表示時に最新版を選ぶ

公開後に質問の最新版が増えるだけでSurveyの内容が変わるため採りません。Surveyは必ず版を固定します。

### AnswerをQuestionnaire domainだけに保存する

アンケート回答もBrain Itemの根拠になり得る生データであり、Source Recordの粒度として既に確定しているため採りません。

### スキップをAnswerとして保存する

スキップは本人についての命題内容を持たず、Brain Itemの根拠になりません。Source Recordを作ると「未回答」という操作と本人の回答内容が同じデータ集合へ混ざるため採りません。

### 回答のたびに新しいSurveyResponseを作る
