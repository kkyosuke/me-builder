...
| Sensitivity | 機微度を表す |
| Source | 本人入力、インポート、AI推定などの由来を表す |
...
`AIが推定したValue`と`本人が明言したValue`は、同じValue型でも信頼性が異なります。分類だけでなく、情報の由来、確信度、本人の確認状態を評価します。具体的な推定・確認モデルは後続で設計します。
...
人は矛盾したValueやPreferenceを持ちます。上書きして1つに統合せず、利用場面、時点、確信度、根拠とともに保持します。
...
Identityは基本プロフィール、Capabilityはプロフィール上のスキル、Behavior Styleは本人が確認したAI推定として簡易的に扱います。複雑なTrade-offやDecision Policyの自動生成は、十分な根拠情報が集まった後に追加します。
