...
ホームはリッチメニューの「ホーム」の遷移先を曖昧にしないために必要です。上部ラベルを「私をひもとく」、見出しを「わたしの診断」とします。Phase 1では回答できる診断を画像サムネイル付きカード形式の2列で一覧表示し、プロフィール分析などを置きません。カードを選択した後の詳細には、対象の診断回答UIだけを表示します。
