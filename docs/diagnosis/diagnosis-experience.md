...
一覧上部にはタブで画面を分断せず、「すべて」と、現在一覧に存在するRelationship Categoryの絞り込みチップを置きます。初期状態は「すべて」とし、選択中のカテゴリは`aria-pressed`でも伝えます。絞り込み後も回答状態によるセクション分けと並び順は維持し、該当する診断がない場合は空であることを表示します。
