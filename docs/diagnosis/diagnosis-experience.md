...
診断一覧、回答、回答結果、案内、読み込みなどの全画面で、ライトテーマとダークテーマを切り替えられるようにします。従来の配色はダークテーマとして残し、初回表示の既定値にもします。切替操作は画面の状態にかかわらず同じ位置から利用でき、現在とは反対のテーマへ変更する操作だと支援技術にも伝わる名前を持たせます。

利用者が選んだテーマはブラウザへ保存し、同じブラウザで次回開いたときに復元します。Phase 1ではAccountへ保存せず、端末間の同期とOSの配色設定への自動追従は行いません。色だけに意味を持たせず、どちらのテーマでも文字、境界、フォーカス表示を判別できるコントラストを保ちます。
...
Phase 1のリッチメニューは次の1項目で開始します。画像全体を1つのタップ領域とし、診断一覧へ遷移させます。

| 項目 | 遷移先 | 役割 |
| --- | --- | --- |
| 診断を行う | LIFFのホーム（診断一覧） | 過去・現在の診断を探す |

「診断を行う」を今日の診断へ直接遷移させると、回答済みの内容や過去の回答を探せません。そのためリッチメニューは一覧を表示するホームへ遷移させ、LINE通知を今日の診断への最短導線にします。Phase 1ではホームが診断一覧だけを持つため、同じ画面へ遷移する「ホーム」と「診断」を分けません。
...
- LINEへ対象診断の通知を配信できる
- リッチメニューの「診断を行う」が正しいLIFF URLを開く
