...
データ取得中のSkeleton UIは遷移先の配置に合わせます。チラつきを避けるため、診断詳細・結果のSkeletonは読み込み開始から最低400ms表示します。400msの計測と通信を並行して進め、表示時間は400msと通信時間の長い方にします。通信完了後にさらに400msを加算しません。診断一覧では画像サムネイル付き2列カード、導入では画像と説明カード、回答画面では進捗・質問カード・選択肢、回答結果では結果見出し・傾向の軸・回答内容の配置を示し、導入、回答画面、回答結果へ遷移している間に診断一覧のカードを表示しません。
...
- スタック奥にある次の質問も、表裏の対であれば2つのラベルを表示する。裏面は表面と異なる配色にして、反転後の面を視覚的に区別できるようにする
- 表面のドラッグ中はカードを左右へ平行移動させず、選択方向へY軸回転を傾けて反転を予告する。回答時はカードを画面外へ退場させず、選んだ左右の方向から回転して裏面を表示する。裏面の回答時は通常の質問と同じく選んだ方向へカードを退場させ、次の質問へ進む
...
- 表裏質問から計算した傾向は、同じParameterの両端ラベルを使い、「普段の行動」と「大切にしたいこと」の2本の軸を上下に並べる。両方を1つの位置へ平均せず、観点ごとの位置と帯域を表示する
- 表裏の両方を計算できる場合は、2つの帯域と位置から「同じ傾向の範囲」「大切にしたいことが高い側」「普段の行動が高い側」のいずれかを中立な文章で表示する。「矛盾」「理想との乖離」のような評価や、回答していない理由の推定を加えない
- 表裏の一方が回答不足なら2つの関係を表示せず、まだ比較できないことを示す。回答中は途中の傾向を表示せず、裏面の回答を誘導しない
