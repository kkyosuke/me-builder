...
| 項目 | 遷移先 | 役割 |
| --- | --- | --- |
| 私を知る | LIFFの「わたしの傾向」 | これまでの回答を横断して自分の傾向を振り返る |
| 診断を行う | LIFFのホーム（診断一覧） | 過去・現在の診断を探す |

左の「私を知る」は「わたしの傾向」へ、右の「診断を行う」は診断一覧へ直接遷移させます。「診断を行う」を今日の診断へ直接遷移させると、回答済みの内容や過去の回答を探せません。そのため診断側は一覧を表示するホームへ遷移させ、LINE通知を今日の診断への最短導線にします。
