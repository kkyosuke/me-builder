...
実行時の採点設定と共通計算はAPI Serverが所有し、保存済み回答の取得時に計算済み結果を返します。Web UIは質問・採点設定・診断IDの対応表を保持せず、診断詳細APIの質問と回答内容APIの計算結果を表示します。
...
5. 最低回答充足率と表示境界を設定する
6. APIの採点設定へ診断IDと重みを追加し、代表回答、全回答、未回答、逆方向、版違いをテストする
...
共通エンジンはWeb UI内の純粋関数です。Answerはサーバーへ保存し、回答内容APIで取得したQuestion ID、Question Version、Choice IDから表示のたびに結果を再計算します。計算済みのParameter Profile自体は保存せず、Brain Itemの生成、改訂時の再計算は行いません。
