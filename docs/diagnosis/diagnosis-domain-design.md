...
| `general` | 自分自身 | 特定の相手との関係を前提にしない質問だけで構成する |
...
受付終了や公開停止の後も、既に1件以上を保存した本人は保存済み回答を確認できます。ただし回答途中のResponseへ追加入力したり、途中回答から結果を生成したりはできません。公開停止は誤ったDiagnosisを緊急停止するための状態であり、内容を変更して再公開する操作には使いません。
