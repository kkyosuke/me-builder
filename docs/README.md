...
- [Web初回起動体験設計](product/web-startup-experience.md) — Web機能リンクから要求画面または利用規約を操作できるまでの段階表示と通信優先順位
...
- [MCP連携設計](architecture/mcp-integration-design.md) — 管理者限定MCPのtransport、認証・認可、同意、開示、監査、解除、仕様更新と停止境界
- [LINE写真日記入力設計](architecture/photo-diary-input-design.md) — LINE写真日記の取得、検証、原本・派生物、容量、安全性、AI利用、削除の境界
- [Web認証・アプリケーションセッション設計](architecture/web-authentication-design.md) — LIFF／SSOを同じAccountとprovider非依存sessionへ収束させる認証境界と移行順序
...
- [開発用本人入力データAPI契約](development/personal-data-api.md) — 開発環境だけの入力確認、日記訂正・削除と診断回答のread-only境界
- [開発用本人データ特徴JSON契約](development/personal-data-export.md) — 本文を含めないBrain特徴取得とdev限定書き出しの境界
...
- [課金監視・復旧Runbook](development/billing-operations-runbook.md) — Stripe課金の監視、問い合わせ初動、再照合、DLQ対応、Secret rotation
