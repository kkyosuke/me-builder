...
変更中は、対象機能のテストや手動確認など、変更範囲に応じた検証を実行します。実行した内容はPRの「動作確認・検証手順」へ記載してください。

push時にはpre-push hookが、変更の影響を受けるworkspaceの型チェックと、E2Eを除く関連テストを自動実行します。全単体テスト、E2E、ビルドなどのフル検証はPR作成後のGitHub Actionsをマージ条件とし、通常のPR作成前にローカルで`task ci`を実行する必要はありません。

`task ci`は、CI障害のローカル再現など、フル検証が必要な場合に限って任意で使用します。
...
- 実行した対象テストや、ローカル環境での手動確認結果を記述します。
