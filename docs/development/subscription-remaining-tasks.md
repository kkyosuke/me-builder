...
[本人入力データの訂正・削除](personal-data-api.md)と`SUB-B-002`はPlanに依存しない本人データ機能です。Planに関係なく常に利用可能であることをB系列の認可testへ含めるため、残るエクスポートをこの系列で管理します。A系列の完成を待たずに着手できます。
...
flowchart TD
    ...
    I --> L[Lite機能側<br/>本人データ訂正・削除 / SUB-B-002〜SUB-B-007 / SUB-B-016]
    L --> U[Full機能側<br/>SUB-B-008〜SUB-B-011 / SUB-B-016]
    U --> F[ファミリー機能側<br/>SUB-B-012〜SUB-B-016]
    F --> R[段階公開<br/>SUB-B-017]
```

決済側は`SUB-A-001`〜`SUB-A-020`で単独検証します。Liteは本人データの訂正・削除、`SUB-B-002`〜`SUB-B-007`、`SUB-B-016`、Fullはさらに`SUB-B-008`〜`SUB-B-011`、ファミリーパックはさらに`SUB-B-012`〜`SUB-B-015`を完了してから、`SUB-B-017`で決済側と統合します。未完了のPlanは購入対象へ出しません。
...
依存: [本人入力データ訂正・削除API契約](personal-data-api.md)
...
依存: 本人データの訂正・削除、`SUB-B-002`〜提供するPlanに必要なB系列タスク
