...
[本人入力データの訂正・削除](personal-data-api.md)と[本人データの特徴取得](personal-data-export.md)はPlanに依存しない本人データ機能です。Planに関係なく常に利用可能であることをB系列の認可testへ含めます。
...
決済側は`SUB-A-001`〜`SUB-A-020`で単独検証します。Liteは本人データの訂正・削除・特徴取得と関係性を考慮した質問、Fullはさらに確認済み履歴を使う関係性質問、ファミリーパックはPlanと個人内容を分離して検証してから、`SUB-B-017`で決済側と統合します。未完了のPlanは購入対象へ出しません。
...
- 支払総額表示、課税区分、請求時期、日割り、支払失敗時の猶予を公開前に専門家と確認する
- 14日間trialは開始時の決済手段登録を必須とし、終了日、終了後価格、自動更新を開始前に表示する
- 利用者都合の返金は行わず、二重・誤請求と重大障害を個別対応する。通常の請求書・領収書はStripeで発行し、適格請求書は発行しない
- 氏名、住所、電話番号は常時公開せず、購入判断前の請求へ遅滞なく個別提供する
- 課金情報はStripeメールとWebだけで案内し、LINEへ送信しない
...
当面は同じサービス管理者が監視、問い合わせ、DLQ復旧を担当します。対応はベストエフォートとし、営業時間、回答期限、可用性を保証しません。安全側への新規購入停止は管理者が即時実行でき、Previewの再確認checklistを完了した後、同じ管理者が再開を承認できます。

### SUB-A-020 PreviewでAccountとPlanの紐付けを検証する

依存: `SUB-A-001`〜`SUB-A-019`

- [Preview検証手順](subscription-preview-plan-verification.md)の`PREVIEW-BILLING-001`〜`010`を共有Previewで実施し、識別子を含まない結果をPRへ残す
...
- 少数招待、一般提供の順に進め、売上、返金、契約、支払失敗はStripe Dashboardで必要時に確認する

完了条件は、Productionで新規購入だけを安全に停止・再開でき、運営Accountの実取引をStripe Dashboard、本人画面、入金明細で突合できることです。30日・90日の独自集計や数値閾値は公開条件にしません。
