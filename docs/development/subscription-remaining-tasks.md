...
採用判断は2026-08-15時点の次の公式情報を前提とします。外部サービスの料金、制約、API versionは変更されるため、`SUB-001`、`SUB-007`、`SUB-035`で再確認します。
...
```mermaid
flowchart TD
    A[提供条件と課金設計<br/>SUB-001〜SUB-006] --> B[Stripe課金基盤<br/>SUB-007〜SUB-014]
    B --> C[購入・契約管理<br/>SUB-015〜SUB-021]
    C --> D[Lite一般提供<br/>SUB-022〜SUB-023]
    D --> E[Full一般提供<br/>SUB-024〜SUB-027]
    E --> F[ファミリーパック<br/>SUB-028〜SUB-031]
    F --> G[リリース検証と運用<br/>SUB-032〜SUB-036]
```

Liteは`SUB-001`〜`SUB-023`、Fullは`SUB-024`〜`SUB-027`も完了してから一般提供します。ファミリーパックは個人向けFullを安定提供した後、`SUB-028`〜`SUB-031`を完了してから追加します。未完了のプランは購入対象へ出さず、公開ページでは準備中または構想中として扱います。

## 5. 提供条件と課金設計

### SUB-001 商取引・税務・返金条件を確定する
...
### SUB-002 課金・利用権限の実装設計を追加する

依存: `SUB-001`

- Stripeと共有D1の正本境界、CustomerとAccount、ファミリー支払者の対応を設計する
- Stripeの契約状態からPlan、Entitlement、利用可能期限へ変換する状態遷移を定義する
- Webhook、Queue、共有D1、API、AccountDataをまたぐ障害時の収束方法を定義する

完了条件は、後続PRが独自の契約状態や利用可否判定を作らずに実装できる課金実装のSSoTがあることです。

### SUB-003 有料契約を復旧できる本人確認を設計する
...
### SUB-004 有料契約のAccount復旧フローを実装する

依存: `SUB-003`, `SUB-009`

- 設計済みの本人確認とIdentity再接続をAPIとWebへ実装する
- 復旧後も同じ契約projection、利用量、本人データへ接続する
- 他Accountへの誤接続と再送による二重統合をnegative testで防ぐ

完了条件は、LINE Account喪失を想定したE2Eで、有料契約を復旧または解約できることです。

### SUB-005 本人データの訂正・削除を有料化条件まで完成させる

依存: なし

- 診断、日記、Source Record、Brain Itemに残る未実装の訂正・削除経路を完成させる
- 結果をチャット文脈、生成物、Vectorize、相性共有へ収束させる
- 課金状態にかかわらず利用でき、既存の残タスクとの重複はリンクで追跡する

完了条件は、料金プラン設計の有料化前条件を自動テストとPreviewで確認できることです。

### SUB-006 本人データのエクスポートを実装する

依存: `SUB-005`

- 本人へ返せるデータ範囲、非同期生成、期限付きダウンロードを設計・実装する
- 他者の非共有情報、内部運用情報、Stripeの支払情報を含めない
- Freeへのダウングレード後も利用できるようにする

完了条件は、代表Accountのエクスポートとデータ境界をE2Eで確認できることです。

## 6. Stripe課金基盤

### SUB-007 Stripe sandboxと商品catalogを再現可能にする
...
### SUB-008 Stripe adapterとエラー契約を追加する
...
### SUB-009 共有D1へ課金projectionを追加する
...
### SUB-010 Billing QueueとDLQを環境へ追加する
...
### SUB-012 Webhookから契約projectionを収束させる
...
### SUB-014 共通Entitlement判定を実装する

依存: `SUB-009`, `SUB-012`

- Accountから現在Plan、利用可能期限、ファミリー由来権限を解決するdomain serviceを追加する
- Webhook遅延、projection不明時の安全側の扱いを統一する
- API、Web、WorkerへStripe statusやPrice IDを公開せず、全Planをtable-driven testで確認する

完了条件は、後続機能が1つのinterfaceだけで利用可否と上限を判定できることです。

## 7. 購入・契約管理

### SUB-015 Checkout Session作成APIを実装する

依存: `SUB-007`〜`SUB-009`, `SUB-014`
...
### SUB-017 Customer Portal導線を実装する

依存: `SUB-007`〜`SUB-009`, `SUB-014`

- 本人だけが自分のCustomerに対する短命なPortal Sessionを作れるAPIを追加する
- 支払方法、請求履歴、Plan変更、期間末解約を決定事項どおり設定する
- ファミリー参加者には支払者用Portalを開かせず、反映待ちを扱う

完了条件は、別AccountのCustomerへ到達できず、本人が支払方法更新と解約予約を完了できることです。

### SUB-018 14日間トライアルを1回だけ開始できるようにする

依存: `SUB-001`, `SUB-009`, `SUB-012`, `SUB-015`

- Accountごとの使用済み状態をCustomer再作成やファミリー移動で回避できない形で保存する
- 開始前に終了日時、終了後の価格、自動更新の有無を表示する
- 終了予告、終了、課金成功・失敗をWebhookから反映し、日時境界をテストする

完了条件は、使用済み・終了直前・終了後・ファミリー移動で2回目を防げることです。

### SUB-019 Plan変更と請求失敗の状態遷移を完成させる

依存: `SUB-012`, `SUB-014`, `SUB-017`, `SUB-018`
...
### SUB-020 AI利用量の予約・確定・解放を実装する

依存: `SUB-014`

- 上限対象を安定したusage kindとして定義し、AccountDataで期間別にatomic管理する
- 同じrequestのretryで二重消費せず、予約、成功時の確定、開始前失敗時の解放を扱う
- 更新日、Plan変更、Free復帰でも履歴を失わず、個人内容を共有D1へ集約しない

完了条件は、並行要求、timeout、期間境界でも上限を超えず、利用量が負数にならないことです。

### SUB-021 利用権限を機能と画面へ接続する
...
### SUB-022 Liteの週次振り返りを実装する

依存: `SUB-014`, `SUB-020`, `SUB-021`
...
### SUB-023 Free・Liteの関係性を考慮した質問を実装する
...
### SUB-024 Fullの確認済み過去情報を関係性質問へ接続する
...
### SUB-026 Fullの行動フォローアップを実装する

依存: `SUB-020`, `SUB-021`, `SUB-024`
...
### SUB-028 ファミリー席のドメインと永続化を追加する

依存: `SUB-002`, `SUB-009`, `SUB-014`, Full一般提供
...
### SUB-033 Stripe契約ライフサイクルの自動E2Eを追加する
...
### SUB-034 課金の監視・サポート・復旧手順を追加する

依存: `SUB-012`, `SUB-013`, `SUB-019`, `SUB-020`

- Webhook、Queue / DLQ、projection遅延、再照合差分、支払失敗を監視する
- 売上、返金、手数料、有料Account、AI変動費を個人内容なしで確認できるようにする
- 二重請求疑い、誤Plan、解約不能、Account復旧、Secret rotationのrunbookを追加する

完了条件は、代表的な問い合わせと障害を検知・判断・復旧できることです。

### SUB-035 Previewで課金リリース判定を行う

依存: 提供するPlanに必要な全タスク、`SUB-032`〜`SUB-034`

- Stripeの最新料金、Portal制約、API version、Webhook eventを公式情報で再確認する
- 購入、更新、支払失敗、解約、購入復元、利用上限をPreviewで通し確認する
- Account復旧、訂正、削除、エクスポート、安全案内、未提供機能の表示を確認する

完了条件は、有料化前条件の証跡と未解決事項がレビューされ、Production開始可否を判断できることです。

### SUB-036 Productionを段階的に公開して価格を検証する
...
- 1番号が大きすぎる場合は、既存番号を変えず`SUB-012A`のような枝番で分割する
- 新しい作業は既存番号を振り直さず末尾へ追加する
