...
- [サブスクリプション・料金プラン設計](../product/subscription-plan-design.md)で確定した支払総額、請求、返金、支払失敗時の扱いを専門家と確認する。[特定商取引法に基づく表記の設計](../product/service-site-design.md#74-特定商取引法に基づく表記)で確定した販売業者名と適格請求書発行事業者の登録番号が、公開表示、Checkout、領収書、請求書で一致することも確認する
- 同設計で確定したトライアル条件が、決済画面と通知で一致することを確認する
- 最終確認画面、特定商取引法に基づく表示、規約、プライバシーポリシーの変更点を確定する

完了条件は、税務・法務の専門家による確認結果が料金プランのSSoT、Checkout、公開文書へ反映され、初回課金時の表示と請求に不一致がないことです。
...
- Stripe同期が出力するコード不要Customer PortalのログインURLを問い合わせ・解約案内から到達可能にし、Accountへログインできない購入者が購入時のメールアドレスでワンタイムパスコードを受け取り、期間末解約できることをPreviewで確認する

完了条件は、別AccountのCustomerへ到達できず、Accountへのログイン可否にかかわらず購入者本人が支払方法更新と解約予約を完了できることです。
