...
`SUB-B-001`と`SUB-B-002`はPlanに依存しない本人データ機能ですが、Planに関係なく常に利用可能であることをB系列の認可testへ含めるため、この系列で管理します。A系列の完成を待たずに着手できます。
...
flowchart TD
    ...
    I --> L[Lite機能側<br/>SUB-B-001〜SUB-B-007 / SUB-B-016]
    L --> U[Full機能側<br/>SUB-B-008〜SUB-B-011 / SUB-B-016]
    U --> F[ファミリー機能側<br/>SUB-B-012〜SUB-B-016]
    F --> R[段階公開<br/>SUB-B-017]
```

決済側は`SUB-A-001`〜`SUB-A-020`で単独検証します。Liteは`SUB-B-001`〜`SUB-B-007`と`SUB-B-016`、Fullはさらに`SUB-B-008`〜`SUB-B-011`、ファミリーパックはさらに`SUB-B-012`〜`SUB-B-015`を完了してから、`SUB-B-017`で決済側と統合します。未完了のPlanは購入対象へ出しません。
...
### SUB-B-001 本人データの訂正・削除を有料化条件まで完成させる
...
### SUB-B-002 本人データのエクスポートを実装する

依存: `SUB-B-001`

- 本人へ返せるデータ範囲、非同期生成、期限付きダウンロードを設計・実装する
- 他者の非共有情報、内部運用情報、決済事業者の支払情報を含めない
...
### SUB-B-003 共通Entitlement判定を実装する

依存: `SUB-A-002`で定義した境界

- `AccountPlanAssignment`から現在Plan、利用可能期限、ファミリー由来権限を解決するdomain serviceを追加する
- Plan紐付けの反映遅延、取得失敗、不明状態の安全側の扱いを統一する
- fake providerで全Planをtable-driven testし、Stripe statusやPrice IDを参照しないことを確認する

完了条件は、A系列のStripe実装が未完成でも、後続機能が1つのprovider非依存interfaceだけで利用可否と上限を判定できることです。
...
## 11. ブランチB: Lite・Fullの継続価値

### SUB-B-006 Liteの週次振り返りを実装する

依存: `SUB-B-003`, `SUB-B-004`, `SUB-B-005`
...
依存: `SUB-B-004`, `SUB-B-005`, `SUB-B-008`
...
依存: `SUB-A-002`で定義した境界、`SUB-B-003`、Full一般提供
...
依存: `SUB-B-001`〜提供するPlanに必要なB系列タスク
