...
flowchart TD
    ...
    L --> U[Full機能側<br/>SUB-B-011 / SUB-B-016]
    U --> F[ファミリー機能側<br/>SUB-B-012〜SUB-B-016]
    F --> R[段階公開<br/>SUB-B-017]
```

決済側は`SUB-A-001`〜`SUB-A-020`で単独検証します。Liteは本人データの訂正・削除・エクスポート、関係性を考慮した質問、`SUB-B-016`、Fullはさらに確認済み履歴を使う関係性質問と`SUB-B-011`、ファミリーパックはさらに`SUB-B-012`〜`SUB-B-015`を完了してから、`SUB-B-017`で決済側と統合します。未完了のPlanは購入対象へ出しません。
