...
2026-08-20時点では、一覧・質問詳細・回答保存・「あとで回答」・回答内容取得がWebからD1まで接続済みです。受付中は保存済み回答を読み込み、回答途中の診断を最初の未回答から再開できます。受付終了後は保存済み回答を閲覧できますが、追加入力と途中回答からの結果生成はできません。`/diagnosis/:diagnosisId`は`liff.state`からも解決され、認証・規約同意後に対象診断を直接開きます。
