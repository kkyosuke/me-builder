...
画面と遷移の正は[Phase 1 診断体験設計](../diagnosis/diagnosis-experience.md)、集約と不変条件の正は[Phase 1 診断ドメイン設計](../diagnosis/diagnosis-domain-design.md)、HTTP契約の正は[診断API契約](diagnosis-api.md)、永続化への写像の正は[`packages/lib`のcatalog schema](../../packages/lib/src/shared-d1/schema/catalog.ts)と[diagnosis schema](../../packages/lib/src/account-data/schema/diagnosis.ts)です。この文書は、それらの定義や具体的なAPIスキーマを所有しません。
