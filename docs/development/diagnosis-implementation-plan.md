...
2026-08-06時点では、一覧・質問詳細・回答保存・「あとで回答」・回答内容取得がWebからD1まで接続済みです。保存済み回答を読み込み、回答途中の診断を最初の未回答から再開できます。
...
現状、Webは`liff.state`のpath判定はあるものの対象診断を開くルートがなく、通知想定のリンクを開いても常に一覧ホームへ落ちます。LINE通知の配信も未実装です。

- 対象診断を直接開く入口をWebへ追加する
- 通知のCTAから対象診断のLIFF URLへ遷移できるようにする
