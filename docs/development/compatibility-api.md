...
現在は一覧参照の件数に上限がなく、成立中の関係ごとに双方の共有内容を読み直して`readiness`を組み立てます。関係数が増えると、1回の一覧取得から発生するAccountData RPCと共有内容の再計算が線形に増えるため、運用データを見ながら一覧のページングまたは関係数の上限を導入し、同じ閲覧者・カテゴリの共有内容をリクエスト内で再利用する必要があります。キャッシュを導入する場合も、共有終了やプロフィール更新を古い状態で表示しないという現在の整合性を維持します。
