...
CREATE INDEX `brain_item_access_label_lookup_idx` ON `brain_item_access_labels` (`label`,`is_deleted`);--> statement-breakpoint
...
CREATE INDEX `brain_item_evidence_item_idx` ON `brain_item_evidence_edges` (`brain_item_id`,`is_deleted`);--> statement-breakpoint
CREATE TABLE `brain_item_revisions` (
	...
	`derivation_method` text NOT NULL,
	FOREIGN KEY (`previous_brain_item_id`) REFERENCES `brain_items`(`id`) ON UPDATE no action ON DELETE no action,
	FOREIGN KEY (`next_brain_item_id`) REFERENCES `brain_items`(`id`) ON UPDATE no action ON DELETE no action
);
--> statement-breakpoint
...
CREATE INDEX `brain_item_topic_label_lookup_idx` ON `brain_item_topic_labels` (`label`,`is_deleted`);--> statement-breakpoint
...
CREATE INDEX `compatibility_reference_status_updated_idx` ON `compatibility_references` (`status`,`updated_at`);--> statement-breakpoint
...
CREATE TABLE `diagnosis_brain_projection_heads` (
	...
	FOREIGN KEY (`current_brain_item_id`) REFERENCES `brain_items`(`id`) ON UPDATE no action ON DELETE no action
);
--> statement-breakpoint
...
--> statement-breakpoint
CREATE UNIQUE INDEX `account_data_identity_account_id_unique` ON `account_data_identity` (`account_id`);--> statement-breakpoint
