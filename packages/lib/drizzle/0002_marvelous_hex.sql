CREATE TABLE `source_record_revisions` (
	...
	FOREIGN KEY (`previous_source_record_id`) REFERENCES `source_records`(`id`) ON UPDATE no action ON DELETE no action,
	FOREIGN KEY (`next_source_record_id`) REFERENCES `source_records`(`id`) ON UPDATE no action ON DELETE no action
);
...
CREATE UNIQUE INDEX `survey_answer_source_record_idx` ON `survey_answers` (`source_record_id`);