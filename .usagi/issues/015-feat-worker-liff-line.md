...
updated_at: 2026-07-26T13:50:23.408751+00:00
---

## 背景

Issue #11 で LIFF の最小疎通を入れたが、**LINE のチャット画面から LIFF を開く導線が存在しなかった**。`apps/worker` は受信したテキストをオウム返しするだけで、LIFF の URL を送っていない。リッチメニューの実装もリポジトリ内に無い。

[プロジェクト概要 §4](../../docs/product/project-overview.md#4-想定する利用体験) の設計では、LINE が「受け付けたことを返信する」→「『今日の診断』のリンクを配信する」流れになっている。オウム返しはこの設計より前のスケルトン挙動なので、設計に合わせる。

## やったこと

- `apps/worker` の返信を、オウム返しから「受け付けた旨 + 診断への LIFF リンク」へ変更
