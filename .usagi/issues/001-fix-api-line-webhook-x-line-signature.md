...
updated_at: 2026-07-26T07:01:11.748120+00:00
---
...
1. `LINE_CHANNEL_SECRET` を環境変数・設定として追加
   ...
   - CD ワークフロー（`cd-preview.yml` / `cd-production.yml`）で `wrangler secret put` により `apps/api` へ配布
...
| 環境 | 挙動 |
| --- | --- |
| `production` | 検証をスキップせず全リクエストを 401 で拒否（`logger.error`） |
| 上記以外 | `logger.warn` を出して検証をスキップ |
