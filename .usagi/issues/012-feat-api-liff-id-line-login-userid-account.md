...
あわせて [プロジェクト概要 §5](../../docs/product/project-overview.md#5-アカウントと本人識別) が「Messaging API と LINE Login の識別子を同じ Account へ束ねられるか」を要確認事項として残していた。
...
結論は [プロジェクト概要 §5](../../docs/product/project-overview.md#5-アカウントと本人識別) へ確定として反映済み。あわせて「**チャネルを同一プロバイダー配下に置くことがアカウント体系の前提**」であることも明記した（プロバイダー間でチャネルは移動できないため、チャネル追加時に確認が必要）。
...
- **`providerAccountId` のログ出力**: 本人識別子をログへ出していた箇所を削除した（[§8](../../docs/product/project-overview.md#8-プライバシーと安全性)）
...
- **サーバー発行のセッション**: ID トークンを毎回送る形のままだと、リプレイ耐性も認可の土台も弱い。書き込み系のエンドポイントを作る前にセッション方式（[ドメイン設計](../../docs/domain/domain-design.md)で未決定）を決める
