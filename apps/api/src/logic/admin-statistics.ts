...
type Params = {
  ...
  getGeminiUsage?: typeof sharedD1.action.geminiUsage.summarizeGeminiUsage;
...
export async function getAdminStatistics(params: Params): Promise<AdminStatisticsOutcome> {
  ...
  const geminiPromise = (params.getGeminiUsage ?? sharedD1.action.geminiUsage.summarizeGeminiUsage)(
    params.db,
    start,
    now,
  );
