 ...
 */
function createTestDb(): sharedD1.Client {
  const sqlite = new Database(":memory:");
  const db = drizzle(sqlite, { schema: sharedD1.schema });
  ...
  return db as unknown as sharedD1.Client;
}
...
describe("createLiffSession", () => {
  ...
  it("友だち追加で作られた Account を引き当て、表示用の情報だけを返すこと", async () => {
    const followed = await sharedD1.action.account.upsertIdentity(db, {
      provider: "line",
    ...
    expect(await db.select().from(sharedD1.schema.accounts).all()).toHaveLength(1);
    const identities = await db.select().from(sharedD1.schema.accountIdentities).all();
  ...
  it("2 回目の呼び出しでも identity が増えないこと", async () => {
    await sharedD1.action.account.upsertIdentity(db, { provider: "line", providerAccountId: SUB });
    mockVerifyEndpoint({ json: validClaims });
    ...
    expect(await db.select().from(sharedD1.schema.accountIdentities).all()).toHaveLength(2);
  });
  ...
  it("ID トークンの検証に失敗した場合は unauthenticated を返し、Account を作らないこと", async () => {
    ...
    expect(result.type).toBe("unauthenticated");
    expect(await db.select().from(sharedD1.schema.accounts).all()).toHaveLength(0);
  });
