...
function answer(questionNumber: number, choiceId: "yes" | "no", version = 1): DiagnosisAnswer {
  const suffix = String(questionNumber).padStart(2, "0");
  return {
    kind: "answer",
    diagnosisQuestionId: `dq-relationship-priority-${suffix}`,
...
describe("RELATIONSHIP_PRIORITY_QUESTIONS", () => {
  it("version 1のYes／No質問を10問持つこと", () => {
    ...
    expect(
      new Set(RELATIONSHIP_PRIORITY_QUESTIONS.map(({ diagnosisQuestionId }) => diagnosisQuestionId))
        .size,
    ).toBe(10);
