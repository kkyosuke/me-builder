...
function response(status: "checking" | "verified", retryAfterMilliseconds = 10) {
  return {
    state: {
      currentAvatar: null,
      job: {
...
describe("useAvatarSettings", () => {
  ...
  it("処理中はRetry-After相当の間隔で再取得し、完了状態で停止する", async () => {
    ...
    await waitFor(() => expect(result.current.job?.status).toBe("verified"));
    expect(mocks.fetchAvatarState).toHaveBeenCalledTimes(2);
    expect(acquireIdToken).toHaveBeenCalledTimes(2);
