  ...
  const accountDataNamespace = {
    getByName: () => ({
      execute: (accountId: string, operation: string, ...args: unknown[]) => {
        ...
        return DO.account.action.diary.attachMessagesToTurn(
          {} as DO.account.Database,
          accountId,
          ...(args as Parameters<typeof DO.account.action.diary.attachMessagesToTurn> extends [
            unknown,
            unknown,
            ...infer TRest,
          ]
...
describe("ConversationCoordinator recovery", () => {
  it("D1反映中のbatchへ後着messageを混ぜない", async () => {
    ...
    const attach = vi
      .spyOn(DO.account.action.diary, "attachMessagesToTurn")
  ...
  it("alarmの外部I/Oが失敗しても次回実行を予約する", async () => {
    ...
    vi.spyOn(DO.account.action.diary, "attachMessagesToTurn").mockRejectedValue(
  ...
  it("保持期間後の古いeventを既存Turnへ再投入しない", async () => {
    ...
    vi.spyOn(DO.account.action.diary, "attachMessagesToTurn").mockResolvedValue({
      turnId: "old-turn",
      sessionId: "session-1",
      generationEpoch: 0,
    });
