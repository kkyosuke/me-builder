...
vi.spyOn(sharedD1.action.account, "upsertIdentity").mockResolvedValue({
  account: {
    id: "account-1",
    status: "active",
    role: "user",
...
describe("Worker", () => {
  ...
  it("AccountDataの失敗を安全に分類し、同じ相関IDで再試行を記録する", async () => {
    ...
    await handleQueueBatch(batch, {} as sharedD1.Client, getWorkerConfig({ ENVIRONMENT: "test" }), {
      d1: {} as sharedD1.Client,
      do: { conversation: coordinatorNamespace, accountData: accountDataNamespace },
      queue: { chatTurn: undefined, brainCheckpoint: undefined },
    });
