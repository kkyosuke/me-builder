...
- **タスク実行とコマンド (`Taskfile.yml`)**:
  ...
  - **CI/CD ワークフロー構造 (`.github/workflows/*.yml`)**:
    ...
    - `deploy-gcp-platform.yml` はGCP platform Pulumi Stackのpreviewまたはapplyを手動実行します。検証済みの`main`、対象と操作に一致する確認文字列、共通`infra` Environmentの承認、GitHub OIDC / Direct Workload Identity Federationを必須とし、service accountとJSON keyを受け付けません。state bucketとManaged Folderは手動の初期構築前提とし、workflowから変更しません。必要な変数・IAM・実行方法は[`infra/gcp-platform/README.md`](../../infra/gcp-platform/README.md#github-actions-deployment)を正とします。
