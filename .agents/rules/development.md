...
- **タスク実行とコマンド (`Taskfile.yml`)**:
  ...
  - **CI/CD ワークフロー構造 (`.github/workflows/*.yml`)**:
    ...
    - `setup-pulumi-state.yml` は既存のstate用GCP projectへGCS bucketとManaged Folderを冪等に初期構築します。`main`からの手動実行、確認文字列、`infra-dev` Environmentの承認、GitHub OIDC / Direct Workload Identity Federationを必須とし、service accountとJSON keyを受け付けません。必要な変数・IAM・実行方法は[`infra/README.md`](../../infra/README.md#one-time-state-backend-bootstrap)を正とします。
