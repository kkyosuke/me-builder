...
- **タスク実行とコマンド (`Taskfile.yml`)**:
  - 開発タスクはルートの `Taskfile.yml` 経由で実行します。
    ...
    - `bun run test:unit`: E2Eを除外したテスト実行
  ...
  - **CI/CD ワークフロー構造 (`.github/workflows/*.yml`)**:
    ...
    - `reset-preview-migrations.yml` は Actions 画面または `gh workflow run reset-preview-migrations.yml --ref main -f confirmation=reset-preview` からだけ起動します。検証済みの`main`を基準に Preview D1 のCloudflare予約table以外（`d1_migrations`を含む）と全 Durable Object namespaceを削除し、同じD1 database resourceへD1 migration、診断seedを再適用してWorker / API / MCPを再デプロイします。全Previewデータを復元不能に削除するため確認文字列と`infra-dev` Environmentの承認を必須とし、productionは対象にしません。`cd-preview`と同じconcurrency groupで直列化します。Pulumi stateは`dev` EnvironmentのGitHub OIDC / Workload Identity Federationで認証した共有GCS backendを使い、Provider側でもrepository ID、`main`、workflow、Environmentを制限します。具体的な設定は[`infra/README.md`](../../infra/README.md#one-time-state-backend-bootstrap)を正とします。
      - 再作成でresource IDが変わった場合は`chore/preview-resource-ids-<run id>`ブランチへcommitしてPRを作成し、同じjobで`bun run ci`が通ったときだけsquash mergeします（`main`はrulesetで直接pushできないため）。検証が落ちたときはPRを開いたまま残します。
    ...
    - `deploy-gcp-platform.yml` はGCP platform Pulumi Stackのpreviewまたはapplyを手動実行します。検証済みの`main`、対象と操作に一致する確認文字列、Developmentでは`infra-dev`、Productionでは`infra-prd` Environmentの承認、GitHub OIDC / Direct Workload Identity Federationを必須とし、service accountとJSON keyを受け付けません。state bucketとManaged Folderは手動の初期構築前提とし、workflowから変更しません。必要な変数・IAM・実行方法は[`infra/gcp-platform/README.md`](../../infra/gcp-platform/README.md#github-actions-deployment)を正とします。
