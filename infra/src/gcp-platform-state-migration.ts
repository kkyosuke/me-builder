...
export type PreparedGcpPlatformStateMigration = {
  deployment: string;
...
/**
 * 旧Vertex credential実装のstateだけを削除可能にする。
 * Pulumiのprotectに加えてprovider側のPREVENTも解除し、次のupで実体を削除する。
 */
export function prepareGcpPlatformStateMigration(
  stackExport: string,
): PreparedGcpPlatformStateMigration {
